...
Multiplicity two contains `3,840` signatures and `7,680` selectors. The first `1,390` signatures, global cases `0` through `1389`, are classified:

- `2,779` multiplicity-two selectors are certified infeasible;
- case `1287`, selector zero, supplies a second verified no-three embedding;
- the classified prefix uses `512,472,441` certified rejection-CSP nodes.
...
- `verify_product_side_seven_multiplicity2_cases1360_1369.py`, `verify_product_side_seven_multiplicity2_cases1370_1379.py`, and `verify_product_side_seven_multiplicity2_cases1380_1389.py` for the selector-aware continuation.
...
1. Continue multiplicity two from case `1390` with selector-aware classification.
...
6. Continue the side-ten fine-row frontier from pair index `4800` and the finite-range, non-affine recursion, protected-spread, bounded-barrier repair, and carry/absorber fronts.
