...
matching reductions PX89--PX99. It records an exact all-side object and
...
| Exact sparse reservoir | PX102 gives `Omega(n/sqrt(log n))` whole rectangles whose union is exactly no-three. |
...
| Nonlinear switching evidence | PX98 gives a four-row strong-complete trade and PX99 proves rank-three spread at order thirteen. |
| Infinite exact closure | **OPEN.** |

## Exact reservoir theorem

PX100 chooses one universal rectangle state with
...
PX101 applies rank-sensitive alteration to the rectangle defect hypergraph.
PX102 retains at least
...
Condition on the PX102 reservoir and expose the unused rectangle/product cells.
...
PX98 supplies a concrete nonlinear four-trade inside the strong-complete
mapping space, and PX99 shows that the full order-thirteen solution space
already has the required rank-three scale. The missing step is an asymptotic
switching-expansion theorem.
...
The PX102 reservoir can be installed before either completion mechanism.
Because it is already no-three, the completion only has to avoid:
...
> After retaining a suitably thinned PX102 reservoir and protecting all
> primitive directions through height `H`, the unused host has normalized
> unavailable-cell, anchored-pair, and candidate-triple loads below the PX90 or
> clone-selection threshold.

The theorem must optimize the reservoir size and height cutoff jointly. The
known bounds suggest using a reservoir smaller than the full PX102 guarantee
when necessary; every subset of the reservoir remains exact and no-three.
