...
proves RI2k and RI3a. A target colour \(C\) acts on the source quotient
by the involution \(A\mapsto rCA^{-1}\), and two successive colours act
by multiplication by their ratio. Even quotient cycles therefore obey
an alternating colour-product identity; an odd colour word determines
its start up to the unique order-two ambiguity. More globally, if \(L\)
is generated by target-colour ratios, every connected source component
lies in two \(L\)-cosets and its image colours lie in one. Thus a large
source component forces large colour-ratio subgroup, while a small
ratio subgroup already gives the simultaneous source/image coset
configuration needed for absorber conversion after density is proved.
RI3b sharpens the same statement by parity. In a bipartite component,
the colour-ratio subgroup is exactly generated by the within-side
source ratios. In a nonbipartite component, an odd cycle collapses the
two source cosets to one, and the subgroup is exactly the full
source-ratio subgroup. Hence the two-source-coset exception survives
only on the bipartite side of the quotient relation.
