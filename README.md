...
2. For each recurrent simple macro cycle, prove current payment, strict bounded field descent, physical impossibility or a capacity-one cycle ticket; also bound the remaining denominator, owner-token, protected-contract and envelope alphabets polynomially.
3. For every pivot, BDA, RI, target and petal menu, bound the protected-filter survivor current-band expectation below `|B|` and prove stepwise bank stability.
4. Obtain common-parent/private-payment petal families or classify their exact parent/payment reset, then discharge AC6.
