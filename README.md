...
1. Classify active and latent saturated-literal roles through exact phase/carry/BDA/RI/protected charts, and discharge or ticket the fixed-registry dependency cycles without charging prospective geometry.
2. For each recurrent simple macro cycle, prove current payment, strict bounded field descent, physical impossibility or a capacity-one cycle ticket; also prove polynomial bounds for the remaining outer alphabets.
3. Audit every pivot, BDA, RI, target and petal menu at scale `H` by proving `N_{2H}=0`, `N_H<|B|` and persistence after intermediate switches.
