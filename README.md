...
A rigorous notebook for attempts toward the classical conjecture
...
> **Status:** The conjecture remains open as of 25 July 2026. This repository does
> not contain a complete proof. It records proved lemmas, conditional reductions,
> failed pathways, exact diagnostics, and explicit next targets.

## Start here

- [`STATUS.md`](STATUS.md): current honesty ledger.
- [`tracks/all-n-prime-patching.md`](tracks/all-n-prime-patching.md): active all-`n`
  roadmap.
...
Every theorem is labelled **PROVED**, **PROVED UNDER HYPOTHESES**,
**CONDITIONAL**, **HEURISTIC**, or **REFUTED**.

## Slab-optimal all-n architecture
...
After deleting direct recapture and unary-shadow cells, matching failure is an
exact Hall rectangle. Binary shadow is governed by endpoint-resource congestion;
its fractional cover problem has factor-two rounding and an exact weighted dual.

A recapture-dominated Hall core yields target-rich nonaxis lines, `Omega(q^3)`
alternating rectangle candidates, and an `Omega(q)` row/column-disjoint rectangle
bank. Superregular residual matching, source validity, finite-state installation,
and diffuse paid selection are closed.

Cross-block states bypass every obstruction supported only on the original two
rectangle diagonals. Equitable colouring and superregular spread absorb every
...
Relative to any perfect matching, orient each allowed index edge `i -> j`.
Perfect matchings factor exactly over the strongly connected components.
...
## Pool-compatible dynamic Xi trades

The dynamic excess potential is
...
The scripts use Python 3.10+ and the standard library.
