...
Fix one connecting line and one corner type of `g`.  By PX67, at most
...
`O(n^3)` follows from PX68 with `h(e,f)>=1`. \(\square\)
...
codegree of rank-three conflicts, while PX69 gives codegree of the same order as
the base degree.
...
partners.  The second assertion is PX68 with `h(e,f)>=H`. \(\square\)
...
exact completion count through base seven, and checks PX68 on deterministic
