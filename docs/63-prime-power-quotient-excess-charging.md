...
which is the claimed expression.  Notice that a vertical cross-layer pair is
distinct at scale `s`, has `q_s(e)=0`, and therefore contributes nothing, as it
should. ∎
...
Fix one unordered modular triple.  It has three choices for the quotient
endpoint pair.  Each quotient point has exactly `t` selected full lifts in its
...
The inherited primitive line signature can only reduce this multiplicity; the
bound does not discard any valid contribution.
...
and sample the balanced recursive bank of CMR67 independently at every node.
The arguments CMR68--CMR74 use successive divisibility of the determinant by
powers of `p`; they therefore apply unchanged to determinant zero modulo the
current modulus, not only to exact real determinant zero.

Triples with three distinct quotient columns have expected modular-zero count
less than

\[
4s m^2+\frac{p+2}{3}m^2.
\]
...
For the balanced recursive quotient,

\[
\mathbb E Z_s(\bar S_s)
<
\left(4s+\frac{p+8}{3}\right)m^2.
\]
...
Apply the modular form of the CMR74 clustering sum to triples with three
distinct columns and add the deterministic vertical-pair population.  Then use
CMR86 and `t^2m^2=N^2`. ∎

## 5. Scale-summed rank-one endpoint

### Corollary CMR89 — PROVED

At one scale in the balanced recursive bank,
...
\sum_{a,\ell}\frac{T_1(A_{s,a,\ell})}{t}
<
\left(6k(k-1)+(p+10)(k-1)\right)N^2.
\]

In particular, one saturated state in the balanced recursive bank has total
rank-one prefix collateral
...
The original quotient-incidence charging target is therefore complete at the
quadratic-polylogarithmic scale.  The remaining prefix-repair obstruction is
now entirely in the normalized rank-two and rank-three terms from CMR78, or in
obtaining a sharper-than-first-moment treatment of the modular quotient
syndrome.
