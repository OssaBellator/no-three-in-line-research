...
Indeed the new normal is `(A,(p+1)B)`, and its common divisor divides `p+1`.
CMR31 therefore bounds one carry cell by
...
After division by `H`, the energy at exact height `H` is at most
`4N(L_H-1)`. Sum this for `H<=K`; every remaining pair contributes at most
`1/K`. Taking `K=floor(sqrt(N))` proves the asymptotic estimate. ∎
...
height-weighted pair sum is exactly `mathcal E(S)`. Insert CMR57. ∎

This is still above the near-linear syndrome sought by the recursive repair
programme. It does close the absence of any full companion bound and shows
that the mixed-layer carry complexity is at most quadratic polylogarithmic for
each fixed prime base.
