...
CMR561 leaves one static object requiring a separate monotone payment: a
pairwise cell-disjoint packing of rank-zero candidate-only conflict triples on
distinct lines. Such a packing has an exact matching-preserving response.

Process its triples inside one deletion pass. If a triple contains a
nonessential edge, delete one such edge and kill the triple. If no edge is
deletable, all three edges are essential. Essentiality persists, and the final
essential core is a matching of size at most the parent side. Because the
packed triples are edge-disjoint, at most one third of the core can support
fully forced packed triples.
...
core by CMR577. The packed triples are pairwise edge-disjoint, so these
...
Let `G_*` be the final host of the deletion pass. Restore an arbitrary edge set
...
presence of `C` after restoration therefore requires `e_C\in R`. CMR579 makes
...
### Corollary CMR581 — PROVED

Apply the pass to the pairwise cell-disjoint rank-zero packing supplied by
CMR561. At least one of the following exact outcomes holds.
...
Combine CMR577--CMR580. ∎

## 6. Revised frontier

The disjoint-support rank-zero branch is paid.
...
Together with CMR570 and CMR576, the remaining prime-power frontier is payment
for repeated protected-core contacts and reuse of fixed wall, heavy-prefix,
dispersed-carry, low-height-line, and secant-star certificates. Every object is
owned by one canonical selector state, so no charge is anonymous.
