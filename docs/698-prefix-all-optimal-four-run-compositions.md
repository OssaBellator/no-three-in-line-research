...
The coordinate embedding depends only on the deletion class, ordered optimal
route, and ordered composition, not on which minimum-crossing matching produced
the route. Deduplicating the 14,976 occurrences leaves exactly

```text
3,624 distinct routes
```

in each deletion class. Thus all physical cases are covered by auditing 7,248
...
The all-`n` theorem remains open, and the next theorem identifier is `PP3dfd`.
