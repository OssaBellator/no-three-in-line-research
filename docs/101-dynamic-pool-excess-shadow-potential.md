...
set `Y_i`, both of size `R`.  Let the complete movement and refill label sets
both have size `T`.
...
positive summands with bad entries.  Each positive summand is at least one, so
the number of positive summands is at most their sum.  Equality to zero is
exactly absence of additional nonaxis blocker pairs. ∎
...
Proposition PP3ku keeps `V_cell` and its constant cardinality fixed.  Expand the
pair-weight sum before and after the trade.  Pairs wholly in the unchanged
source cancel, leaving exactly the removal-credit and insertion-cost terms. ∎
...
Assume that whenever the current source has positive-density bad cell entries,
the PP3hx star/resource alternative supplies a source-admissible pool-compatible
trade satisfying
...
Then finitely many such trades produce a source with `o(1)` bad cell-entry
density.  If the conversion hypothesis applies whenever `Xi>0`, the process
terminates at `Xi=0`.

#### Proof

Every trade preserves `V_cell` and strictly decreases the nonnegative integer
`Xi`.  Hence no state repeats and only finitely many trades occur.  If the
process stopped while the stated density condition still held, the conversion
hypothesis would supply another decreasing trade.  The final assertions follow
from PP3kw. ∎
...
supports.  The remaining obstruction is geometric conversion of that host or
its Hall rectangle, not dynamic relabelling of controller edges.
