...
For example, when \(\eta_m>0\), one may take the integer part of

\[
\min\{m^\kappa,\eta_m^{-1/2}\}
\]

after an immaterial lower truncation tending to infinity. When \(\eta_m=0\),
take the integer part of \(m^\kappa\).

### Proposition PP3nq -- PROVED

A uniform \(q\)-subset of the \(Q\) endpoint indices has expected unary-edge
count \(o(q)\). It also has expected anchored-transition count \(o(q)\).
...
3. At most \((q'-1)(q'-2)\) directed 3-cycle events use a fixed resource. Each
   has probability \(1/(q')_3\), again totaling at most \(1/q'\).
...
\[
\Pr(\pi(i)=j\mid\operatorname{avoid\ low})
\le
\dfrac{e^{24/q'}}{q'}.
\]
...
\[
\Pr(\pi(i)=j\mid\operatorname{complete\ source\ validity})
\le
\dfrac{1+o(1)}{q'}
\]
...
\[
\Pr(E\mid\mathcal G,\operatorname{avoid\ low})
\le
\dfrac{\Pr(E\mid\operatorname{avoid\ low})}
{\Pr(\mathcal G\mid\operatorname{avoid\ low})}
\le
\dfrac{e^{24/q'}}{q'(1-o(1))}
=
\dfrac{1+o(1)}{q'}.
\]
...
\((1+\epsilon_{q'})/q'\), where \(\epsilon_{q'}=o(1)\), and fixed-point
