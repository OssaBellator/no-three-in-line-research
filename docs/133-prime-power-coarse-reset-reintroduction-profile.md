...
uses each such row exactly once.  Apply CMR378.  A retained deletion mask can
...
### Theorem CMR380 — PROVED UNDER THE DESCENDING PREFIX SCHEDULE
...
so there is one compatible block per relevant moving layer.  CMR379 bounds each
such reset by `t/p^b`.  Sum with multiplicity.  CMR378 gives equality before
...
### Theorem CMR381 — PROVED UNDER THE ONE-PASS HYPOTHESIS
...
once.  Apply CMR380 and then CMR347. ∎

The token-labelled aggregate is also explicit.

### Corollary CMR382 — PROVED UNDER THE ONE-PASS HYPOTHESIS
...
CMR381 contributes at most `2bt/p^b` to each token.  Hence the depth-`b` sum is
at most
...
The multiplicity assumption gives `A_tau<=2bm`.  Apply CMR380 and CMR347.  The
...
CMR383 converts unbounded edge return into repeated use of one fixed ancestor
...
need a separate profile; CMR378 remains available there at the full envelope
scale.
