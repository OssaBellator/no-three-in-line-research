...
and let `r,s` be nonnegative integers satisfying
...
The number of nonvertical, nonhorizontal real lines containing at least `n-r`
points of
...
first `r+s+2` positions. The condition `r\le n-2` guarantees that these first
two used positions exist.
...
As in CMR228, every cell of `A\times C` except `uv` is absent from `H`, and the
...
The threshold on `h` gives `n\ge h+1-r\ge r+2`, so CMR234 applies. At most

\[
2\binom{r+2}{2}\binom{3r+2}{2}=M_r
\]

of the lines can meet `A\times C` in at least `n-r` cells. Every other line has
...
CMR241 and CMR245 subsequently supersede this count, proving simultaneous
avoidance through `t-3` full-derangement lines and `t-2` target-specific lines.
CMR234--CMR236 remain useful as an independent near-transversal mechanism and
as a possible template for structured blockers beyond the sharp boundary
threshold.
