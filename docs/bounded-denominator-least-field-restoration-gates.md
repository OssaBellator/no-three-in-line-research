...
## BDA5bl -- canonical least changing field -- PROVED
...
## BDA5bm -- canonical leave/restore excursion -- PROVED
...
BDA5bl supplies at least one exit.  The chosen rotation makes the least exit first.  Finite cyclic return supplies a first later `a`-vertex.  Minimality of `r` gives the strict avoidance statement for the interior vertices. QED.

The address is canonical after fixing the coordinate orders, value orders, edge order and cycle rotation convention.

## BDA5bn -- monotone physical fields forbid recurrence -- PROVED
...
The restoration excursion leaves `a` and later returns to `a`.  A monotone sequence returning to its starting value is constant throughout, contradicting the strict interior avoidance in BDA5bm. QED.
...
## BDA5bo -- capacity-one restoration tickets close fixed-profile cycles -- PROVED UNDER THE RESTORATION-TICKET CONTRACT
...
Every recurrent nonconstant cycle has one canonical restoration gate by BDA5bm.  The contract assigns a distinct unused capacity-one resource to each permitted traversal and forbids reuse.  The finite reachable restoration-edge stock bounds the number of traversals. QED.

## BDA5bp -- least-field cycle router -- PROVED UNDER THE COMPLETE-PROFILE CONTRACT

Every simple same-denominator recurrent physical-profile cycle has one of the following continuations:

1. its least changing field is monotone, so the cycle is impossible by BDA5bn;
...
Apply BDA5bl--BDA5bm.  Monotone fields use BDA5bn.  The other declared outcomes are mutually nonexclusive valid continuations for the exact restoration edge.  If none applies while the physical interpretation changes, completeness fails and the least omitted field is returned as an outer reset. QED.
