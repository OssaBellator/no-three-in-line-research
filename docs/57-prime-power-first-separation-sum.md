...
and then sums it over all column triples.  For primes congruent to one modulo
four, a larger balanced reciprocal family gives the natural quadratic-logarithmic
syndrome scale.
...
This is impossible for a nonsquare `c`.
...
Hence every cell occurs in exactly `h` of the `ph` states.

Two prescribed cells in distinct columns determine both parameters.  If one
column is zero, it fixes `b` and the other fixes `c`.  If both are nonzero,
subtracting
...
determines `c`, and then `b`.  Thus at most one state survives; additional cells
cannot increase the count. ∎

Using `B_p` independently at every layer-prefix node gives a recursive
saturated bank with

\[
(ph)^{2(1+p+\cdots+p^{k-1})}
=
(ph)^{2(N-1)/(p-1)}
\]

states.  Every one-fibre triple is excluded by the same determinant argument as
CMR45.

## 2. A multiscale first-separation probability

The next statement applies to any recursive digit bank whose local node law has
a one-cell bound.  At depth `n`, the node key of a selected position is
...
most `mu`, independently at distinct nodes.
...
\[
\Pr\bigl(\text{the three selected points are real collinear}\bigr)
\le
\mu^{k-s-1}.
\]
...
If `Delta=0`, then after division by `p^r` the partial linear combination is
zero modulo every power of `p`.  Conditional on divisibility modulo `p^n`, the
next congruence has the form

\[
C_n+K_j\eta_{j,n}\equiv0\pmod p,
\]

once all other depth-`n` outputs are exposed.  Because `K_j` is a unit, at most
one value of `eta_{j,n}` can satisfy it.

For every depth `n>s`, the three column prefixes modulo `p^n` are pairwise
distinct.  Therefore the chosen point's node key is used by no other point in
the triple.  Its local output has conditional atom at most `mu`, and the nodes
at different depths are independent.  There are exactly

\[
k-s-1
\]

such depths, namely `s+1,...,k-1`.  Multiplying their conditional bounds proves
the theorem. ∎

The estimate deliberately ignores the separation depth `s` itself.  At that
depth two or three points may still use one local map, so a one-cell argument is
not universally available.  Discarding one level costs only a constant factor
in the global sum.
...
has eight layer assignments.  Triples using two points in one column are
vertical with respect to every third point and never contribute.

### Corollary CMR69 — PROVED

For the original restricted conic bank CMR43, whose one-cell atom is
`mu=1/h`,
...
\le
4N^3h^{-(k-1)}
\sum_{s=0}^{k-1}\left(\frac hp\right)^s
<
\frac{4N^3h^{-(k-1)}}{1-h/p}
<
8\frac{N^3}{h^{k-1}}.
\]
...
For every fixed odd `p>=5`, this is a genuinely subcubic syndrome bound.

If `p=1 mod 4` and the balanced family `B_p` is used at every node, then
`mu=1/p` and
...
Consequently, for every prime power `N=p^k` with `p=1 mod 4`, the balanced
recursive bank contains a saturated state with at most

\[
4pkN^2=O_p(N^2\log N)
\]

real collinear triples, while retaining exact product spread and deterministic
exclusion of every one-fibre triple.

### Proof

Insert CMR68 into the preceding count and sum over `s`.  For the original bank,
...
Since `h/p<1/2`, the displayed bounds follow.
...
A state no worse than the expectation exists. ∎
...
near-linear repair threshold.  The remaining improvement must exploit more than
one scalar divisibility condition per geometric certificate, or use cancellation
and codegree structure beyond a raw triple first moment.
