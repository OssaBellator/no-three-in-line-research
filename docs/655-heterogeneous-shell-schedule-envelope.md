...
The earlier constant-burden shell law permits a fixed recurring cost. A geometric
macro may instead have period-dependent repair and collision costs. This chapter
gives the exact prefix criterion for an arbitrary burden sequence and a complete
classification for periodic schedules.
...
The constant-burden law is the special case `m=1`.
...
unbounded cumulative saving. The values `delta_i` and `c_i` have not been derived
from coordinates and exposed-state repairs.
