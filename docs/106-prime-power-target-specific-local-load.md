...
\[
\frac{\mathbf1_{v\in V(z_*)}}t+rac{T_3(v)}{(t)_3}>\frac1{24}.
\]
...
target-specific parent permutation avoiding every candidate cell on those
`m` lines.
...
certificate can use a removed line because all of its board cells are absent. ∎
