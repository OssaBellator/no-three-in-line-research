...
## PP3der — Forced-alphabet legality at every width
...
listed `Pi` layers themselves are a legal decomposition.

Therefore, for every window width,

```text
a forced-alphabet endpoint is legal iff its window contains no identity layer.
```

The four-layer classification in `docs/689` is the width-four instance of this
general separator theorem.

## PP3des — Sharp cyclic formula for arbitrary width
...
be the lengths of the intervening identity-free gaps. Then
...
legal cyclic windows of width `w`. Hence the total is

```text
sum_i max(0,g_i-w+1).
```

For fixed total gap length this sum is maximized by concentrating all `5K` legal
positions in one gap. Thus the exact optimum is
...
In particular, once `w>5K`, every rolling window is illegal.

## PP3det — Exact finite audit and uniqueness of optimizers
...
from one through `8K`. The maximum agrees with `max(0,5K-w+1)` in every case.
For
...
Thus wider rolling memory supplies no new concealment mechanism. Any threshold
advance must leave the forced minimum alphabet or use a genuinely unexposed,
non-rolling operation.
...
are never exposed as endpoint matrices.

The all-`n` theorem remains open, and the next theorem identifier is `PP3deu`.
