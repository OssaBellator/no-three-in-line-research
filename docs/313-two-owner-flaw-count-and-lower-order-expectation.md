...
and three-owner flaws, while `docs/312` solves the fixed-cycle two-owner
orientation subsystem as a parity CSP.  This chapter counts the full two-owner
atomic event family and shows that it contributes only linear expected defect
mass under the uniform signed Hamilton measure.
...
Choose the singleton owner's source pair in `m` ways and one of its eight signed
cell roles: two orientations times four orbit positions.  By PP3bla, as its
target varies that cell moves along one axis-parallel line.  A nonaxis line `L`
intersects that parameter line in at most one point, so at most one target pair
can complete the collinearity.
...
### Corollary PP3bld -- PROVED / FRONTIER REFINED
...
PP3blc gives the two-owner upper bound.  PP3bje gives `Theta(n log n)` expected
strongly generic three-owner triples. ∎

This separation matches both the repair and preprocessing architecture:

```text
two-owner exceptional flaws: parity preprocessing or support-one flips;
three-owner generic flaws:    support-three successor rotations.
```

The finite atomic-flaw audit records exact two-owner flaw counts
...
all far below the cubic bound.  The asymptotic bottleneck remains the residual
three-owner causal process after parity preprocessing.
