...
The exhaustive identity-position censuses for `K=1,2,3` verify the bound for all
widths and contain respectively 56, 8,008, and 1,307,504 position sets. The
checker also verifies the contiguous extremizer for every `K` from one through
64.

## PP3dew — Wider rolling memory cannot conceal the forced identity mass
...
Thus increasing rolling memory by any fixed amount does not improve the
asymptotic legal density beyond the four-window value.
...
The exact audit is `scripts/check_threshold_all_window_widths.py`, included in
