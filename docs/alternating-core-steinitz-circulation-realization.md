...
AC3uc--AC3ug apply a finite-dimensional cone alternative to a coupled additive macro dictionary.  The
unranked branch is a bounded primitive count vector `x>=0` with nonnegative net output `z=Vx>=0`.
That witness is algebraic: an arbitrary ordering of its macros can temporarily overdraw one resource
coordinate even though the total output is nonnegative.

This note removes the ordering obstruction whenever the current resource state has one explicit uniform
buffer.  Append `z_i` virtual unit drains in coordinate `i`, apply the Steinitz rearrangement lemma to the
...
## AC3uh -- Steinitz ordering with one-sided resource deficit -- PROVED
...
## AC3ui -- uniform buffer makes the circulation chronological -- PROVED UNDER THE LEGALITY CONTRACT

If the current resource state satisfies

`m_i>=beta_i`

for every coordinate, the order from AC3uh is a legal chronological execution of all `L` real macros.
...
Before macro `lambda_(t+1)`, AC3uh gives current stock at least
...
## AC3uj -- explicit chronological word and buffer-address stocks -- PROVED
...
## AC3uk -- exact-return and positive-output circulation routes -- PROVED
...
In the first case the complete word is an exact recurrence address.  In the second case the least positive coordinate is an exact net-output field.  The gross macro ledger
retains the occurrence identities of the created units which realize that net output, and those units must
enter an existing cap, source-payment, ticket, monotone-output or outer-reset route.

### Proof

AC3ui gives final state `m+z` with the same finite root-boundary state.  If `z=0`, every field returns
...
## AC3ul -- buffered circulation realization router -- PROVED UNDER THE DECLARED CONTRACTS
...
Consequently the chronological-ordering part of AC3ug is no longer an open interface in buffer-rich
states.  What remains is payment of positive-output circulations and control of histories trapped below
one or more buffer thresholds.

### Proof

Apply AC3uh--AC3uj.  AC3uk classifies the realized final output.  Every excluded hypothesis is retained as
