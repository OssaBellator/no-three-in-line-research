...
namespace {
constexpr int GLOBAL_CASE=4;
const Assignment FULL_TOP={{0,1,6,4,5,2,3,2,1,4,3,6,5,0}};
const std::array<uint64_t,14> EXPECTED_ORDERS={{1,2,4,5,6,9,35,35,43,156,206,752,6888,67044}};
const std::array<uint64_t,14> EXPECTED_TOP_NODES={{2,5,11,23,45,78,374,375,776,2750,4369,18949,144782,196131}};
const std::array<uint64_t,14> EXPECTED_BOTTOM_NODES={{1,3,8,10,12,19,55,55,71,267,366,1213,10439,112185}};
...
struct AssumptionTopEnumerator {
    ...
    void run(Signature const& signature_value,uint16_t keep_mask) {
...
void locate_case(std::vector<State> const& layer,Signature& signature,std::vector<State>& group) {
    ...
    int index=0;
    for(auto const& [sig,candidate_group]:groups) {
    ...
    assert(false&&"missing multiplicity-three case");
}
}
...
int main() {
    ...
    assert(keep_mask==0);
    std::cout<<"empty_top_assumption_core top_orders=67044 top_nodes=196131 bottom_nodes=112185 PASS\n";
}
