...
struct CSP{
 ...
 int group(int v)const{return v<7?0:v<14?1:2;}bool done(int i)const{auto&p=spec[i];return asg[p.col]>=0&&(p.top||asg[14+p.label]>=0);}int id(int i)const{auto&p=spec[i];int r=p.top?p.label:asg[14+p.label];int x=ori<2?(p.top?r:7+r):2*r+(p.top?0:1);int oc=p.col/7,cp=asg[p.col];int y=ori%2==0?7*oc+cp:2*cp+oc;return 14*x+y;}
