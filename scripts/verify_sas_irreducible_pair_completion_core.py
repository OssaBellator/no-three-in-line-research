#!/usr/bin/env python3
"""Audit SAS5ku--SAS5ky: canonical irreducible pair/completion cores."""
from collections import deque
import random
SEED, SYSTEMS = 18007, 4300
...
def sample(rng):
    ...
    d=[rng.randint(1,6) for _ in range(t)]; ty=[rng.randrange(2) for _ in range(t)]
    return p,m,t,pc,pm,mt,d,ty

def capacity(a,mask):
    p,m,t,pc,pm,mt,d,_=a; s=0; ps=1; ms=ps+p; ts=ms+m; sink=ts+t; f=Flow(sink+1)
...
def main():
    rng=random.Random(SEED); z={k:0 for k in ("checks","deficient","addresses","deficit","marginal","single","pair","completion")}
    for _ in range(SYSTEMS):
        ...
        for x in addr:
            ...
            z["pair" if a[7][x]==0 else "completion"]+=a[6][x]
    print("SAS5ku--SAS5ky audit passed")
    ...
    print(f"pair demand units in cores: {z['pair']:,}")
    print(f"completion demand units in cores: {z['completion']:,}")
