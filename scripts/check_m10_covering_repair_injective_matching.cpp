...
int main(){
 ...
 vector<Pair> basePairs;basePairs.reserve(optimalStates);vector<pair<int,int>> zeroSources;long long matchedTotal=0,edgesTotal=0;int failedMasks=0;
 #pragma omp parallel
 {
   vector<Pair> locPairs;vector<pair<int,int>> locZero;long long locMatched=0,locEdges=0;int locFailed=0;
   #pragma omp for schedule(dynamic,1)
   for(int mask=0;mask<Q;mask++){
     vector<int> srcNon;vector<vector<int>> raw;vector<int> targets;srcNon.reserve(sourcesByMask[mask].size());raw.reserve(sourcesByMask[mask].size());
     for(int ri:sourcesByMask[mask]){
       vector<int>a;
       for(uint64_t p=offset[ri];p<offset[ri+1];p++){int y=cleanTarget[p];if(sat[y].test(mask)){a.push_back(y);targets.push_back(y);}}
       if(a.empty())locZero.push_back({ri,mask});
       else{locEdges+=a.size();srcNon.push_back(ri);raw.push_back(move(a));}
     }
     sort(targets.begin(),targets.end());targets.erase(unique(targets.begin(),targets.end()),targets.end());vector<vector<int>>adj(raw.size());
     for(int i=0;i<(int)raw.size();i++){adj[i].reserve(raw[i].size());for(int y:raw[i])adj[i].push_back(lower_bound(targets.begin(),targets.end(),y)-targets.begin());sort(adj[i].begin(),adj[i].end());adj[i].erase(unique(adj[i].begin(),adj[i].end()),adj[i].end());}
     HK hk(adj,targets.size());int got=hk.run();locMatched+=got;if(got!=(int)adj.size())locFailed++;
     for(int u=0;u<(int)adj.size();u++)if(hk.ml[u]>=0){uint64_t targetKey=(uint64_t(targets[hk.ml[u]])<<M)|mask;uint64_t sourceRef=(uint64_t(mask)<<32)|uint32_t(srcNon[u]);locPairs.push_back({targetKey,sourceRef});}
   }
   #pragma omp critical
   {basePairs.insert(basePairs.end(),locPairs.begin(),locPairs.end());zeroSources.insert(zeroSources.end(),locZero.begin(),locZero.end());matchedTotal+=locMatched;edgesTotal+=locEdges;failedMasks+=locFailed;}
 }
 sort(basePairs.begin(),basePairs.end(),[](Pair const&a,Pair const&b){return a.first<b.first;});sort(zeroSources.begin(),zeroSources.end());
 vector<vector<uint64_t>> exceptionAdj(zeroSources.size());long long totalLocal=0;
 for(int i=0;i<(int)zeroSources.size();i++){
   auto [ri,mask]=zeroSources[i];auto &cand=exceptionAdj[i];
   ...
   sort(cand.begin(),cand.end());cand.erase(unique(cand.begin(),cand.end()),cand.end());totalLocal+=cand.size();if(cand.empty()){cerr<<"empty exception adjacency\n";return 12;}
 }
 const uint64_t NONE=~0ULL;unordered_map<uint64_t,uint64_t> changed;changed.reserve(4096);
 ...
 auto fixedAdj=[&](uint64_t sourceRef,vector<uint64_t>&out){int mask=sourceRef>>32;int ri=uint32_t(sourceRef);out.clear();for(uint64_t p=offset[ri];p<offset[ri+1];p++){int y=cleanTarget[p];if(sat[y].test(mask))out.push_back((uint64_t(y)<<M)|mask);}};
 int directFree=0,maxPathSources=0;long long bfsSourceVisits=0,bfsTargetVisits=0;
 for(int ex=0;ex<(int)zeroSources.size();ex++){
   uint64_t start=(1ULL<<63)|uint64_t(ex);vector<uint64_t>q;size_t qh=0;q.push_back(start);
   unordered_map<uint64_t,uint64_t> parentTargetOfSource;parentTargetOfSource.reserve(4096);
   unordered_map<uint64_t,uint64_t> parentSourceOfTarget;parentSourceOfTarget.reserve(4096);
   unordered_set<uint64_t> seenSource,seenTarget;seenSource.reserve(4096);seenTarget.reserve(4096);seenSource.insert(start);
   uint64_t freeTarget=NONE;vector<uint64_t> temp;
   while(qh<q.size()&&freeTarget==NONE){uint64_t sref=q[qh++];bfsSourceVisits++;vector<uint64_t> const*adjPtr=nullptr;if(sref>>63)adjPtr=&exceptionAdj[sref&~(1ULL<<63)];else{fixedAdj(sref,temp);adjPtr=&temp;}for(uint64_t t:*adjPtr){if(!seenTarget.insert(t).second)continue;bfsTargetVisits++;parentSourceOfTarget[t]=sref;uint64_t occ=occupant(t);if(occ==NONE){freeTarget=t;break;}if(seenSource.insert(occ).second){parentTargetOfSource[occ]=t;q.push_back(occ);}}}
   if(freeTarget==NONE){cerr<<"global augmenting path failure at exception "<<ex<<"\n";return 14;}
   int pathSources=0;uint64_t t=freeTarget;
   while(true){uint64_t sref=parentSourceOfTarget[t];pathSources++;changed[t]=sref;auto it=parentTargetOfSource.find(sref);if(it==parentTargetOfSource.end())break;uint64_t old=it->second;changed[old]=NONE;t=old;}
   if(pathSources==1) directFree++;
   maxPathSources=max(maxPathSources,pathSources);
 }
 ...
 bool ok=optimalStates==12786720&&cleanStates==115586396&&
   cleanTarget.size()==35746320&&matchedTotal==12786646&&
   edgesTotal==423160022&&zeroSources.size()==74&&failedMasks==0&&
   basePairs.size()==12786646&&totalLocal==564&&directFree==34&&
   maxPathSources==2&&bfsSourceVisits==114&&bfsTargetVisits==410&&
   occupiedCount==12786720;
 if(!ok){cerr<<"matching regression mismatch\n";return 13;}
 cout<<"{\n  \"m\":10,\n  \"optimal_positive_signed_states\":"<<optimalStates<<",\n  \"clean_signed_states\":"<<cleanStates<<",\n  \"cycle_level_clean_rotation_labels\":"<<cleanTarget.size()<<",\n  \"fixed_sign_nonexception_sources\":"<<matchedTotal<<",\n  \"fixed_sign_edges\":"<<edgesTotal<<",\n  \"fixed_sign_masks_without_complete_matching\":"<<failedMasks<<",\n  \"fixed_sign_exceptions\":"<<zeroSources.size()<<",\n  \"exception_distinct_local_targets\":"<<totalLocal<<",\n  \"exceptions_assigned_without_rerouting\":"<<directFree<<",\n  \"maximum_augmenting_path_source_count\":"<<maxPathSources<<",\n  \"augmenting_bfs_source_visits\":"<<bfsSourceVisits<<",\n  \"augmenting_bfs_target_visits\":"<<bfsTargetVisits<<",\n  \"global_injective_direct_clean_policy_constructed\":true,\n  \"maximum_reverse_indegree\":1\n}\n";return 0;
}
