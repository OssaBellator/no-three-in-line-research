...
Point certificate_point(int edge,Assignment const& top,std::array<int8_t,N> const& bottom) {
    int row=edge/SIDE;
    int column=edge%SIDE;
    return {row<N ? row : N+bottom[row%N],N*(column/N)+top[column]};
}

std::array<uint8_t,3> first_bad_triple(State const& state,Assignment const& top,std::array<int8_t,N> const& bottom) {
    auto edges=certificate_edges(state);
    for(size_t i=0;i<edges.size();++i)
        for(size_t j=i+1;j<edges.size();++j)
            for(size_t k=j+1;k<edges.size();++k)
                if(det(certificate_point(edges[i],top,bottom),certificate_point(edges[j],top,bottom),certificate_point(edges[k],top,bottom))==0)
                    return {static_cast<uint8_t>(edges[i]),static_cast<uint8_t>(edges[j]),static_cast<uint8_t>(edges[k])};
    assert(false);
    return {};
}

uint64_t saturation_mix64(uint64_t hash,uint64_t value) {return (hash^value)*1099511628211ULL;}
...
int main() {
    ...
    Signature signature{};std::vector<State> group;int case_index=0;
    for(auto const& [candidate_signature,candidate_group]:groups) {
        if(candidate_group.size()!=4) continue;
        if(case_index==1180) {signature=candidate_signature;group=candidate_group;std::sort(group.begin(),group.end());break;}
        ++case_index;
    }
    assert(case_index==1180&&group.size()==4);
    TopEnumerator top;top.sig=signature;top.run(0);assert(top.solutions.size()>=64);
    const std::array<size_t,64> expected_dictionary_sizes = {{
        ...
        364,364,366,369,371,371,376,376,
        380,380,380,385,385,388,389,389,
        391,391,393,395,395,395,395,395,
        395,395,395,395,400,401,404,404
    }};
    ...
    for(int top_index=0;top_index<64;++top_index) {
        ...
        digest=saturation_mix64(digest,top_index);
        for(auto value:top_assignment) digest=saturation_mix64(digest,static_cast<uint8_t>(value));
        ...
        do {
            for(int selector=0;selector<4;++selector) {
                auto triple=first_bad_triple(group[selector],top_assignment,bottom);dictionary.insert(triple);for(auto edge:triple) digest=saturation_mix64(digest,edge);++obligations;
            }
    ...
    size_t projected_bytes=48+64*14+dictionary.size()*3+obligations;
    assert(dictionary.size()==404);assert(obligations==1290240);assert(projected_bytes==1292396);assert(digest==4157547873615595023ULL);
