#!/usr/bin/env python3
"""Audit RI5eo--RI5es: canonical irreducible owner/charge cores."""
from collections import deque
import random
SEED, SYSTEMS = 18003, 4200
...
def main():
    rng=random.Random(SEED); z={k:0 for k in ("checks","deficient","addresses","deficit","marginal","single","owner","charge")}
    for _ in range(SYSTEMS):
        ...
        for x in addr:
            ...
            z["owner" if a[7][x]==0 else "charge"]+=a[6][x]
    print("RI5eo--RI5es audit passed")
    ...
    print(f"owner demand units in cores: {z['owner']:,}")
    print(f"charge demand units in cores: {z['charge']:,}")
