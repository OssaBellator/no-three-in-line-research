#!/usr/bin/env python3
"""Finite audit for BDA5hf--BDA5hi."""
...
def matching(rs,ts):
    ...
    def go(rid,seen):
        for tid in adj[rid]:
            ...
            seen.add(tid)
            if tid not in mt or go(mt[tid],seen): mt[tid]=rid; return True
        return False
    for r in rs: go(r[0],set())
    return len(mt),{rid:tid for tid,rid in mt.items()}
def fibres(xs,n): return {k:[x for x in xs if x[1]==k] for k in range(n)}

def main():
    rng=random.Random(SEED); stats=Counter(); ok=trial=0
    while ok<CASES:
        ...
        rp,tp=r0+dr,t0+dt
        old={(r[0],t[0]) for r in rs for t in ts if compat(r,t)}
        full={(r[0],t[0]) for r in rp for t in tp if compat(r,t)}
        ...
        drk,tpk,r0k,dtk=(fibres(x,nk) for x in (dr,tp,r0,dt))
        exact=sum(len(drk[k])*len(tpk[k])+len(r0k[k])*len(dtk[k]) for k in range(nk)); assert exact==len(boundary)
        ...
        stats.update(boundary=exact,full_pairs=len(rp)*len(tp),deficient=deficit>0,deficit=deficit,churn=a+b); ok+=1
    print(f"BDA keyed epochs: {ok}"); print(f"key-local/full pairs: {stats['boundary']}/{stats['full_pairs']}")
    print(f"deficient updates: {stats['deficient']}"); print(f"total deficit/churn: {stats['deficit']}/{stats['churn']}")
