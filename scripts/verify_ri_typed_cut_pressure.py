#!/usr/bin/env python3
"""Finite audit for RI5ey--RI5fc."""
from collections import deque
from random import Random

SEED, SYSTEMS, TYPED = 2103, 3000, True
...
def flow(n, edges, s, t, enabled):
    ...
    while True:
        ...
        aug = 10**9; v = t
        while v != s:
            u = parent[v]; aug = min(aug, res[u][v]); v = u
        v = t
        while v != s:
            u = parent[v]; res[u][v] -= aug; res[v][u] += aug; v = u
        value += aug
    reach = [False] * n; reach[s] = True; q = deque([s])
    while q:
        u = q.popleft()
        for v in range(n):
            if not reach[v] and res[u][v] > 0:
...
def system(rng):
    ...
    for i in range(np):
        ...
        for j in range(nq):
            if rng.random() < .65:
                edges.append((p0 + i, q0 + j, rng.randint(1, 3), ("pq", i, j))); made = True
        if not made:
    ...
    for j in range(nq):
        ...
        for k in range(nt):
            if rng.random() < .65:
                edges.append((q0 + j, x0 + k, rng.randint(1, 3), ("qt", j, k))); made = True
        if not made:
            k = rng.randrange(nt); edges.append((q0 + j, x0 + k, rng.randint(1, 3), ("qt", j, k)))
...
def enabled(demand, mask):
    return {("term", k): d if mask >> k & 1 else 0 for k, d in enumerate(demand)}


def main():
    rng = Random(SEED)
    out = dict(systems=SYSTEMS, subset_checks=0, deficient=0, singletons=0,
    ...
    for _ in range(SYSTEMS):
        ...
        for mask in range(1, 1 << nt):
            value, _, _ = flow(n, edges, s, t, enabled(demand, mask))
            total = sum(demand[k] for k in range(nt) if mask >> k & 1)
        ...
        if zero and one:
            a = sum(1 << k for k in zero); out["typed_mixed"] += 1
        else:
            a = sum(1 << k for k in members[:len(members) // 2])
        b = core ^ a; assert vals[a][1] == vals[b][1] == 0
        out["nontrivial"] += 1; out["nontrivial_deficit"] += delta
        _, _, reach = flow(n, edges, s, t, enabled(demand, core))
        va, fa, _ = flow(n, edges, s, t, enabled(demand, a)); vb, fb, _ = flow(n, edges, s, t, enabled(demand, b))
        ...
        out["cut_arcs"] += cuts; out["positive_arcs"] += count
        out["positive_pressure"] += positive; out["backward_crossings"] += back
    print("RI typed cut-pressure audit passed")
