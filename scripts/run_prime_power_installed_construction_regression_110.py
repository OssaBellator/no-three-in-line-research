#!/usr/bin/env python3
"""Run the installed one-hundred-ten-kind construction checker stack."""
...
class ConstructionRegression110Error(RuntimeError):
    pass


def require(ok: bool, message: str) -> None:
    if not ok:
        raise ConstructionRegression110Error(message)
...
MANIFEST_EXTENSION = [
    (
        ...
        "packed_conflict_deletion_ancestry_proved",
    ),
    (
        ...
        "installed_transition_kind_bank_110_exhaustive",
    ),
]
EXPECTED_CHAINED_MANIFEST_SHA256 = "c35739fdbfd53f0ae1357113e6b636a2c2b403e8fa6c6ad8cc7def28274cf59f"
...
def repository_root(start: Path | None = None) -> Path:
    ...
    for candidate in (current, *current.parents):
        if (candidate / "STATUS.md").is_file() and (candidate / "scripts").is_dir():
            return candidate
    raise ConstructionRegression110Error("unable to locate repository root")
...
def audit(root: Path, item: tuple[str, str, str], execute: bool) -> dict[str, Any]:
    ...
    try:
        ...
        raise ConstructionRegression110Error(f"{path}: invalid JSON report") from exc
...
def main() -> None:
    ...
    report = {
        "checker": "prime-power-installed-construction-regression-110",
        "base_chained_manifest_sha256": BASE_CHAINED_MANIFEST_SHA256,
