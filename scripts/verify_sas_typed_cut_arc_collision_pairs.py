        ...
        def search(u):
            ...
            for _order, v, key in adjacency.get(u, []):
                if remaining.get((u, v, key), 0) > 0 and v not in visiting:
                    ...
                    if search(v):
                        return True
            visiting.remove(u); return False
        assert search(source)
        nodes, keys, cursor = [sink], [], sink
        while cursor != source:
            previous, key = parent[cursor]; nodes.append(previous); keys.append(key); cursor = previous
        nodes.reverse(); keys.reverse()
        multiplicity = min(remaining[(nodes[i], nodes[i + 1], keys[i])] for i in range(len(keys)))
...
def main():
    ...
    for _ in range(SYSTEMS):
        ...
        type_zero = [k for k in members if types[k] == 0]; type_one = [k for k in members if types[k] == 1]
        if type_zero and type_one:
            side_a = sum(1 << k for k in type_zero); out["typed_mixed"] += 1
        else:
        ...
        paths_a = unit_paths(n, edges, source, sink, flow_a); paths_b = unit_paths(n, edges, source, sink, flow_b)
        capacities = ENABLED(demand, core); positive = 0
        for u, v, capacity, key in edges:
            ...
            if pressure <= 0:
                continue
            through_a = [path for path in paths_a if key in path[1]]; through_b = [path for path in paths_b if key in path[1]]
            assert len(through_a) == flow_a[(u, v, key)] and len(through_b) == flow_b[(u, v, key)]
            assert pressure <= min(len(through_a), len(through_b))
            pairs = list(zip(through_a[:pressure], through_b[:pressure])); class_load = defaultdict(int)
            for a, b in pairs:
                assert a[0] in members and b[0] in members
                class_load[(a[0], b[0])] += 1
            a_classes = len({a[0] for a, _b in pairs}); b_classes = len({b[0] for _a, b in pairs})
            assert max(class_load.values()) * a_classes * b_classes >= pressure
            positive += 1; out["positive_arcs"] += 1; out["collision_pairs"] += pressure
            out["path_pair_checks"] += pressure; out["side_a_path_uses"] += len(through_a)
            out["side_b_path_uses"] += len(through_b); out["max_pressure"] = max(out["max_pressure"], pressure)
        assert positive > 0
        out["nontrivial_cores"] += 1
    print("SAS typed cut-arc collision audit passed")
