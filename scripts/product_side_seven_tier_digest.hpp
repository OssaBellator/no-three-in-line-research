...
inline uint64_t tier_digest_mix(uint64_t hash,uint64_t value) {
    hash ^= value;
    return hash * 1099511628211ULL;
}

inline int verify_tier_digest(
    int argc,
    char** argv,
    TierDigestExpectations const& expected
) {
    ...
    for(auto const& [signature,unsorted_group]:groups) {
        ...
        if(selected) {
            ...
            if(requested_orientation<0) {
                digest=tier_digest_mix(digest,case_index);
                for(auto value:signature) digest=tier_digest_mix(digest,value);
                for(int mode=0;mode<2;++mode) {
                    digest=tier_digest_mix(digest,top[mode].solutions.size());
                    aggregate_top_orders[mode]+=top[mode].solutions.size();
                }
                for(int mode=0;mode<2;++mode) {
                    digest=tier_digest_mix(digest,top[mode].nodes);
                    aggregate_top_nodes[mode]+=top[mode].nodes;
                }
                for(int orientation=0;orientation<4;++orientation) {
                    digest=tier_digest_mix(digest,bottom_nodes[orientation]);
