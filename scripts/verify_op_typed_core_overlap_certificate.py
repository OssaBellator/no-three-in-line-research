#!/usr/bin/env python3
"""Exhaustive finite audit for OP4dz--OP4ed."""

SYSTEMS = 3400
FOUR_TERMINAL_SYSTEMS = 620
PAIR_CORE_SYSTEMS = 1950
SINGLETON_CORE_SYSTEMS = 340
...
def main():
    ...
    for system in range(SYSTEMS):
        ...
        types = ["residual", "edit"] + ["residual" if i % 2 == 0 else "edit" for i in range(2, n)]
        ...
        if core.bit_count() == 1:
            ...
            continue
        nontrivial += 1
        assert {types[i] for i in range(n) if core & (1 << i)} == {"residual", "edit"}
        mixed += 1
        least = core & -core
    ...
    assert subset_checks == 98880
    assert deficient == 2290 and paid == 1110
    assert singleton == 340 and nontrivial == mixed == 1950
    assert addresses == marginal_checks == 4240
    assert deficit_units == 2290
    assert partitions == overlap_units == 1950
    print("OP4dz--OP4ed typed core-overlap audit passed")
    print(f"systems: {SYSTEMS}")
    print(f"terminal-subset checks: {subset_checks}")
    print(f"deficient systems: {deficient}")
    print(f"singleton cores: {singleton}")
    print(f"mixed residual/edit cores: {mixed}")
