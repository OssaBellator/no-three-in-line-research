...
def main():
    ...
    for _ in range(2000):
        ...
        boundary = (
            {(r, t) for r in new_r for t in next_t}
            | {(r, t) for r in stable_r for t in new_t}
        )
        ...
        carried = {
            r: t for r, t in old_assignment.items()
            if r in stable_r and t in stable_t
        }
        assert len(set(carried.values())) == len(carried)
        assert all((r, t) in next_edges for r, t in carried.items())
        disturbed = sum(
            1 for r in stable_r
            if r in old_assignment and old_assignment[r] not in stable_t
        )
    ...
    print(
        f"verified {systems} RI epochs, {boundary_checks} boundary pairs, "
        f"{corrupted} corruptions, {complete} complete and {deficient} deficient updates"
    )
