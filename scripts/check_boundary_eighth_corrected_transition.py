#!/usr/bin/env python3
"""Audit the eighth-step spectrum through six and the unique correction."""
...
for node, offset in product(NODES, range(-RADIUS, RADIUS + 1)):
    kind, variant = node
    block = frozenset((next_x + x, next_y + offset + y) for x, y in VARIANTS[kind][variant])
    ...
    if minimum is not None and minimum <= 5:
        ...
        small_records.append((node, offset, minimum, transversals, block))

assert histogram == {4: 1, 5: 6, 6: 21, ">6": 492}
assert len(small_records) == 7
assert sum(len(record[3]) for record in small_records) == 21

successful = []
for node, offset, minimum, transversals, block in small_records:
    ...
    for transversal_index, core in enumerate(transversals):
        ...
        if correction is not None:
            successful.append((node, offset, minimum, transversal_index, core, correction))

assert len(successful) == 1
node, offset, minimum, transversal_index, core, correction = successful[0]
...
for ninth_node, ninth_offset in product(NODES, range(-RADIUS, RADIUS + 1)):
    ...
    block = {(ninth_x + x, ninth_y + ninth_offset + y) for x, y in VARIANTS[kind][variant]}
...
print({
    ...
    "remaining_gap": "the corrected path continues only after another correction; no raw ninth step exists",
