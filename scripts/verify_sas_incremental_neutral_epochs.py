#!/usr/bin/env python3
"""Finite checks for SAS5mx--SAS5nb."""
...
def main():
    ...
    for _ in range(2000):
        nr = rng.randint(1, 8); nt = rng.randint(nr, nr + 5)
        old_r = [f"r{i}" for i in range(nr)]; old_t = [f"t{i}" for i in range(nt)]
        old_edges = {(r, old_t[i]) for i, r in enumerate(old_r)}
        old_edges |= {(r, t) for r in old_r for t in old_t if rng.random() < 0.35}
        ...
        next_edges = unchanged | {pair for pair in boundary if rng.random() < 0.35}
        for r in new_r:
        ...
        if boundary:
            pair = next(iter(boundary)); bad = set(rebuilt); bad.symmetric_difference_update({pair})
            assert bad != next_edges; corrupted += 1
        ...
        complete += deficit == 0; deficient += deficit > 0
        boundary_checks += len(boundary); systems += 1
    print(f"verified {systems} SAS epochs, {boundary_checks} boundary pairs, {corrupted} corruptions, {complete} complete and {deficient} deficient updates")
