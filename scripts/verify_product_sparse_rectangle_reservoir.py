#!/usr/bin/env python3
"""Verify PR1--PR3: exact sparse rectangle reservoirs."""
    ...
    for side in range(1, 5):
        ...
        for chosen in range(1 << len(edges)):
            pairs = {
                ...
                if len(edge) == 2 and chosen >> index & 1
            }
            triples = {
                edge
                for index, edge in enumerate(edges)
                if len(edge) == 3 and chosen >> index & 1
            }
            independence = maximum_independent_size(side, pairs, triples)
            for numerator in range(21):
                q = numerator / 20
                lower = side * q - len(pairs) * q**2 - len(triples) * q**3
                assert independence + 1e-12 >= lower
    print("PR2 alteration bound: exhaustive through four vertices")
...
def verify_constants() -> None:
    ...
    print("PR3 constants: checked through side 10000")
...
def main() -> None:
    ...
    verify_constants()
    verify_small_states()
    print("PR1--PR3 verification passed")
