#!/usr/bin/env python3
"""Finite audit for BDA5ih--BDA5ik."""
from collections import Counter
from math import ceil
from random import Random


def run(seed=202, systems=2500):
    ...
    for _ in range(systems):
        ...
        mult=Counter(x for f in fs for x in f); eta=max(mult.values(),default=0)
        h=rng.randint(0,min(6,len(p))); edit=set(rng.sample(p,h))
        touched=[i for i,f in enumerate(fs) if f&edit]
        assert len(touched)<=h*eta
        active=set(range(n))
        for i in touched:
            if rng.randrange(3)==1: active.remove(i)
        disturbance=rng.randint(0,6); active.update(range(n,n+disturbance))
        assert len(active)<=n+disturbance and len(active)==len(set(active))
        total=bound=0
...
if __name__=='__main__':
    r=run(); print('BDA queue revalidation audit passed')
