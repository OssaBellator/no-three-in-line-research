TITLE='AC mixed-cut decomposition audit'
SEED=1801
SYSTEMS=5000
STAGES=3
TYPED=False
...
def run():
    ...
    for _ in range(SYSTEMS):
        ...
        caps=[[rng.randint(0,6) for _ in range(n)] for n in sizes]
        demand=[rng.randint(1,6) for _ in range(nt)]; kinds=[0]*nt
        ...
        stage_cut=[sum(caps[k][j] for j in range(sizes[k]) if R[ins[k][j]] and not R[outs[k][j]]) for k in range(STAGES)]
        demand_in=sum(demand[v] for v in range(nt) if R[terms[v]]); demand_out=sum(demand)-demand_in
        assert demand_in+sum(stage_cut)==f
        unpaid=sum(demand)-f; assert unpaid==demand_out-sum(stage_cut)
        ...
        for k,x in enumerate(stage_cut): z[f'stage{k}_cut_units']+=x
