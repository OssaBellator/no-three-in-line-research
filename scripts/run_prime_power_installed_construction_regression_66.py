#!/usr/bin/env python3
"""Run the installed sixty-six-kind construction checker stack."""
...
class ConstructionRegression66Error(RuntimeError):
    pass


def require(ok: bool, message: str) -> None:
    if not ok:
        raise ConstructionRegression66Error(message)
...
def repository_root(start: Path | None = None) -> Path:
    ...
    for candidate in (current, *current.parents):
        if (candidate / "STATUS.md").is_file() and (candidate / "scripts").is_dir():
            return candidate
    raise ConstructionRegression66Error("unable to locate repository root")


def validate_manifest() -> None:
    require(digest(CHECKER_MANIFEST) == EXPECTED_MANIFEST_SHA256,
            "twenty-eight-checker manifest digest mismatch")
    paths: set[str] = set()
    flags: set[str] = set()
    for index, item in enumerate(CHECKER_MANIFEST):
...
def audit(root: Path, item: tuple[str, str, str], execute: bool) -> dict[str, Any]:
    ...
    try:
        ...
        raise ConstructionRegression66Error(f"{path}: invalid JSON report") from exc
    ...
    require(report.get("all_n_proved_by_checker") == 0, f"{path}: honesty flag must remain zero")
    return {
        "path": path,
        "contract": contract,
        "expected_flag": flag,
...
def main() -> None:
    ...
    results = [audit(root, item, execute=not args.static_only) for item in CHECKER_MANIFEST]
    report = {
        "checker": "prime-power-installed-construction-regression-66",
