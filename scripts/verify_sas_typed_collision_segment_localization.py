#!/usr/bin/env python3
"""Deterministic audit for SAS5lo--SAS5ls."""
from collections import defaultdict
from importlib import import_module
from random import Random
BASE=import_module("verify_sas_typed_cut_arc_collision_pairs")
FLOW,UNIT_PATHS,ENABLED=BASE.FLOW,BASE.unit_paths,BASE.ENABLED
SEED,SYSTEMS=2407,2100
...
def main():
    ...
    for _ in range(SYSTEMS):
        ...
        if zero and one: side_a=sum(1<<k for k in zero); out["typed_mixed"]+=1
        else: side_a=sum(1<<k for k in members[:len(members)//2])
        side_b=core^side_a
        ...
        for u,v,c,key in edges:
            ...
            for pa,pb in zip(aa[:pressure],bb[:pressure]):
                common,source_prefix=segment(pa,pb,key); out["collision_pairs"]+=1; out["segment_edges"]+=len(common); out["max_segment_length"]=max(out["max_segment_length"],len(common)); out["source_prefix" if source_prefix else "internal_merge_split"]+=1
                if types[pa[0]]!=types[pb[0]]: out["cross_type_pairs"]+=1
                signatures[(common,types[pa[0]],types[pb[0]])]+=1
        assert signatures; out["nontrivial"]+=1; out["concentrated_pairs"]+=max(signatures.values())
    print("SAS typed collision-segment audit passed")
