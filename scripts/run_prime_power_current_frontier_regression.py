...
It validates research infrastructure and stated finite claims only and
...
FINITE_THEOREM_CHECKS = (
    ...
    "check_prime_power_masked_host_parent_generation.py",
...
DOCUMENT_EXPECTATIONS: dict[str, tuple[str, ...]] = {
    ...
    "docs/11-open-bottlenecks.md": (
        "CMR2839", "run_prime_power_current_frontier_regression.py",
    ...
    "docs/427-prime-power-canonical-frontier-roots.md": (
        "CMR2663", "all_n_proved_by_checker = 0",
    ),
    "docs/428-prime-power-current-frontier-regression.md": (
        "CMR2664--CMR2675", "all_n_proved_by_checker = 0",
    ),
    "docs/429-prime-power-negative-regression-open-fixture.md": (
        "CMR2676--CMR2691", "all_n_proved_by_checker = 0",
    ),
    "docs/430-prime-power-isolated-import-honesty-audit.md": (
        "CMR2692--CMR2705", "Correction:", "all_n_proved_by_checker = 0",
    ),
    "docs/431-prime-power-reproducible-runtime-manifest.md": (
        "CMR2706--CMR2721", "manifest_sha256", "all_n_proved_by_checker = 0",
    ),
    ...
    "docs/438-prime-power-hard-core-population-bridge.md": (
        ...
        "actual_parent_rule_present = 0", "t21_semantic_chambers_proved = 0",
        "all_n_proved_by_checker = 0",
    ),
    ...
    "docs/442-prime-power-canonical-masked-host-parent-generation.md": (
        "CMR2828--CMR2839",
    ...
    for node in tree.body:
        if isinstance(node, ast.Assign):
            ...
            if isinstance(node.target, ast.Name) and node.target.id == name:
                return ast.literal_eval(node.value)
    raise CurrentFrontierRegressionError(
        f"{path}: literal assignment {name} not found"
    )
...
def syntax_inventory(scripts_dir: Path) -> list[str]:
    paths = sorted(
        ...
        if path.name.startswith((
            "check_prime_power_", "verify_prime_power_",
            "run_prime_power_", "test_prime_power_",
        ))
    )
...
def validate_target_frontier_literals(
    target_rows: Any, frontiers: Any
) -> tuple[list[str], list[str]]:
    require(isinstance(target_rows, list) and len(target_rows) == 43,
            "atomic target table must contain exactly 43 targets")
    require(isinstance(frontiers, dict) and len(frontiers) == 13,
            "frontier table must contain exactly 13 groups")
    require(all(isinstance(key, str) and key for key in frontiers),
            "frontier IDs must be nonempty strings")
    require(all(isinstance(value, str) and value for value in frontiers.values()),
            "frontier titles must be nonempty strings")
    target_ids: list[str] = []
    frontier_ids: list[str] = []
    for index, row in enumerate(target_rows, start=1):
        require(isinstance(row, tuple) and len(row) == 10,
                f"target row {index}: exact ten-field tuple required")
        target_id, frontier_id = row[0], row[1]
        require(isinstance(target_id, str)
                and target_id.startswith(f"T{index:02d}_"),
                f"target row {index}: sequential target ID required")
        require(frontier_id in frontiers,
                f"target {target_id}: unknown frontier {frontier_id}")
        target_ids.append(target_id)
        frontier_ids.append(frontier_id)
    require(len(target_ids) == len(set(target_ids)),
            "duplicate atomic target ID")
    require(set(frontier_ids) == set(frontiers),
            "every frontier must contain at least one target")
    return target_ids, list(frontiers)


def exact_target_frontier_census(
    atomic_path: Path,
) -> tuple[list[str], list[str]]:
...
def validate_endpoint_text(filename: str, text: str) -> None:
    ...
    require("all_n_proved_by_checker" in text,
...
def validate_document_markers(
    relative_path: str, text: str, markers: tuple[str, ...]
) -> None:
    require(text.strip(), f"{relative_path}: empty document")
    for marker in markers:
        require(marker in text,
                f"{relative_path}: missing synchronization marker {marker!r}")


def document_audit(root: Path) -> list[str]:
    for relative_path, markers in DOCUMENT_EXPECTATIONS.items():
        validate_document_markers(
            relative_path, source_text(root / relative_path), markers
        )
...
def run_script(
    root: Path, script: str, arguments: tuple[str, ...] = ()
) -> dict[str, Any]:
...
def exact_regression(
    root: Path, static_only: bool = False
) -> dict[str, Any]:
    ...
    self_tests = [] if static_only else [
        run_self_test(root, script, argument)
        for script, argument in SELF_TESTS
    ]
    ...
    bridge_checks = [] if static_only else [
        run_script(root, script, arguments)
        for script, arguments in FRONTIER_BRIDGE_CHECKS
    ]
    claims = {
...
def main() -> None:
    ...
    result = exact_regression(
        repository_root(), static_only=args.static_only
    )
