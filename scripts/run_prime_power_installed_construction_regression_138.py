#!/usr/bin/env python3
"""Run the installed one-hundred-thirty-eight-kind construction checker stack."""
...
class ConstructionRegression138Error(RuntimeError):
    pass


def require(ok: bool, message: str) -> None:
    if not ok:
        raise ConstructionRegression138Error(message)
...
MANIFEST_EXTENSION = [
    (
        ...
        "protected_interface_execution_ancestry_proved",
    ),
    (
        "scripts/check_prime_power_installed_operation_registry_138.py",
        "1ad61f338ba3095e632b4b3e2ee28b1b0400a082d50795311c077068cf6beda3",
        "installed_transition_kind_bank_138_exhaustive",
    ),
]
EXPECTED_CHAINED_MANIFEST_SHA256 = "e04a2859b6d64b738d7e300a4fd42c371ef476aefb73542e4649bda99a8b88b3"
...
def repository_root(start: Path | None = None) -> Path:
    ...
    for candidate in (current, *current.parents):
        if (candidate / "STATUS.md").is_file() and (candidate / "scripts").is_dir():
            return candidate
    raise ConstructionRegression138Error("unable to locate repository root")
...
def load_base_manifest(root: Path) -> list[tuple[str, str, str]]:
    runner117 = load_module(
        root / "scripts/run_prime_power_installed_construction_regression_117.py",
        "installed_regression_117",
    )
    require(
        runner117.EXPECTED_CHAINED_MANIFEST_SHA256 == BASE_CHAINED_MANIFEST_SHA256,
        "base 117-kind chained seal",
    )
    manifest = [*runner117.load_base_manifest(root), *runner117.MANIFEST_EXTENSION]
    runner117.validate_manifest(manifest)
    require(len(manifest) == BASE_MANIFEST_COUNT, "base thirty-four-checker count")
    return manifest
...
def validate_manifest(manifest: list[tuple[str, str, str]]) -> None:
    ...
    require(len(manifest) == BASE_MANIFEST_COUNT + len(MANIFEST_EXTENSION),
            "thirty-six-checker manifest required")
    paths: set[str] = set()
    flags: set[str] = set()
...
def audit(root: Path, item: tuple[str, str, str], execute: bool) -> dict[str, Any]:
    ...
    try:
        ...
        raise ConstructionRegression138Error(f"{path}: invalid JSON report") from exc
...
def main() -> None:
    ...
    report = {
        "checker": "prime-power-installed-construction-regression-138",
        "base_chained_manifest_sha256": BASE_CHAINED_MANIFEST_SHA256,
