...
bool better(Best const&a,Best const&b){if(a.supply!=b.supply)return a.supply>b.supply;return tie(a.c.s,a.c.ta,a.c.tb,a.c.tc,a.c.mask)<tie(b.c.s,b.c.ta,b.c.tb,b.c.tc,b.c.mask);}
int main(){
  ...
  for(int ci=0;ci<C;ci++){
    ...
    if(impossible)continue;Info in;in.comp.fill(-1);in.colour.fill(-1);bool ok=true;int cc=0;
  ...
  auto key=[](int s,int ta,int tb,int tc){return ((s*M+ta)*M+tb)*M+tc;};vector<vector<int>>lists(K*M*M*M);
  ...
  vector<Candidate>cand;cand.reserve(100000);for(int s=0;s<K;s++){auto S=triples[s];for(int ta=0;ta<M;ta++)for(int tb=0;tb<M;tb++)for(int tc=0;tc<M;tc++){if(lists[key(s,ta,tb,tc)].empty())continue;for(int mask=0;mask<4;mask++)if(triple_bad(S[0],ta,mask&1,S[1],tb,(mask>>1)&1,S[2],tc,0))cand.push_back({(uint16_t)s,(uint8_t)ta,(uint8_t)tb,(uint8_t)tc,(uint8_t)mask});}}
  int threads=1;
#ifdef _OPENMP
  threads=omp_get_max_threads();
#endif
  struct Acc{long long flaws=0,incidences=0,supply=0;int maxSources=0;array<long long,721>hist{};Best best;};vector<Acc>acc(threads);
#pragma omp parallel for schedule(dynamic,32)
  for(int ix=0;ix<(int)cand.size();ix++){int tid=0;
#ifdef _OPENMP
    ...
    if(!ns)continue;auto&a=acc[tid];a.flaws+=2;a.incidences+=2LL*ns;a.supply+=2*supply;a.maxSources=max(a.maxSources,ns);a.hist[ns]+=2;Best cb{supply,ns,c};if(better(cb,a.best))a.best=cb;
  }
  ...
  auto quantile=[&](long long n,long long d){long long threshold=(total.flaws*n+d-1)/d,cumulative=0;for(int i=1;i<=720;i++){cumulative+=total.hist[i];if(cumulative>=threshold)return i;}return 720;};
  ...
  vector<char>meet(K*K,0);for(int s=0;s<K;s++)for(int t=0;t<K;t++)for(int a:triples[s])for(int b:triples[t])if(a==b)meet[s*K+t]=1;vector<TrancheAcc>tracc(threads);
#pragma omp parallel for schedule(dynamic,1)
  for(int ix=0;ix<(int)cand.size();ix++){int tid=0;
    ...
    if((int)sources.size()!=720)continue;vector<vector<int>>nbr(sources.size());vector<int>all;all.reserve(61200);
    ...
    long long gs=accumulate(weights.begin(),weights.end(),0LL),gc=accumulate(caps.begin(),caps.end(),0LL),d=gcd(gs,gc),gp=gs/d,gq=gc/d;auto&a=tracc[tid];a.flaws+=2;a.maxCuts=max(a.maxCuts,r.cuts);
