...
def run_case(rng):
    ...
    delta_r_by_key = Counter(key_r[r] for r in r_changed)
    delta_t_by_key = Counter(key_t[t] for t in t_new)
    stable_r_by_key = Counter(key_r[r] for r in r_stable)
    next_t_by_key = Counter(key_t[t] for t in T)

    boundary = sum(
        delta_r_by_key[k] * next_t_by_key[k]
        + stable_r_by_key[k] * delta_t_by_key[k]
        for k in range(key_count)
    )
    lt = max(next_t_by_key.values(), default=0)
    lr = max(stable_r_by_key.values(), default=0)
    ...
    expected_initial = set(r_changed + r_disturbed)
    assert initial_unmatched == expected_initial

    unmatched = set(R) - set(matching)
    assert unmatched <= expected_initial
...
def main():
    ...
    print(
        f"verified {CASES} RI support-local systems; "
