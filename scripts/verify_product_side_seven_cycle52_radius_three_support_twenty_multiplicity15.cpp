...
const std::array<CaseData,2> CASES = {{
    {0, {{257,6,1536,3072,144,8224,4160}}, {{28670,46994}}, {{489174,673720}}, {{79145,122000,80063,131227}}},
    {1, {{257,514,12,24,2176,8224,4160}}, {{45248,38240}}, {{191271,166791}}, {{121026,108081,133296,104217}}}
}};
...
int main(int argc,char**argv) {
    ...
    for (int case_index=0;case_index<(int)CASES.size();++case_index) {
        ...
        auto group=it->second; std::sort(group.begin(),group.end()); assert(group.size()==15);
        TopEnumerator top[2];
    ...
    if(requested_case<0 && requested_orientation<0) { assert(checked==2); assert(aggregate_bottom==879055ULL); std::cout<<"aggregate_bottom_nodes="<<aggregate_bottom<<" PASS\n"; }
}
