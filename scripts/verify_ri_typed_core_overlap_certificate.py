#!/usr/bin/env python3
"""Exhaustive finite audit for RI5et--RI5ex."""
...
def main():
    ...
    for system in range(SYSTEMS):
        ...
        types = ["owner", "charge"] + ["owner" if i % 2 == 0 else "charge" for i in range(2, n)]
        ...
        nontrivial += 1
        assert {types[i] for i in range(n) if core & (1 << i)} == {"owner", "charge"}
        mixed += 1
        least = core & -core
    ...
    assert singleton == 320 and nontrivial == mixed == 1850
    assert addresses == 4020 and deficit_units == 2170
    assert marginal_checks == 4020
    assert partitions == 1850 and overlap_units == 1850

    print("RI5et--RI5ex typed core-overlap audit passed")
    print(f"systems: {SYSTEMS}")
    print(f"terminal-subset checks: {subset_checks}")
    print(f"deficient systems: {deficient}")
    print(f"singleton cores: {singleton}")
    print(f"mixed owner/charge cores: {mixed}")
