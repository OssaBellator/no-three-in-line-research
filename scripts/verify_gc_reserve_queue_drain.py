#!/usr/bin/env python3
"""Finite verifier for GC2nb--GC2ne."""
from __future__ import annotations
import math, random
from dataclasses import dataclass
@dataclass(frozen=True)
class Job: cost: tuple[int,...]
def drain(q,cap,kappa,W,rng):
    reserve=[rng.randint(0,cap[a]) for a in range(len(cap))]; rep=[max(1,math.ceil(kappa[a]/W)) for a in range(len(cap))]
...
def main():
    rng=random.Random(303); systems=jobs=arrivals=oversized=shortages=0
    for _ in range(2500):
        ...
        delta0=rng.randint(n,n+10); U=sum(rng.randint(0,4) for _ in range(rng.randint(2,10))); A=n+rng.randint(0,delta0+U-n); assert A<=delta0+U
        arrivals+=A
        if rng.random()<.12: oversized+=1
        if rng.random()<.25:
            a=rng.randrange(d); supply=max(0,k[a]-rng.randint(1,max(1,k[a]))); assert k[a]-supply>0; shortages+=1
        systems+=1; jobs+=n
    print(f"verified {systems} GC queues, {jobs} jobs, {arrivals} admissions")
