...
namespace {
struct PartialKeyNext {
    uint16_t mask = 0;
    Assignment values{};
    bool operator<(PartialKeyNext const& other) const {
        if (mask != other.mask) return mask < other.mask;
        return values < other.values;
    }
};

PartialKeyNext partial_key_next(uint16_t mask, Assignment const& assignment) {
    PartialKeyNext key;
    key.mask = mask;
    key.values.fill(-1);
...
int main() {
    const std::array<int, 18> top_indices = {
        3,5,8,10,11,32,33,36,37,38,39,40,41,49,52,55,58,62
    };
    ...
    std::set<PartialKeyNext> vocabulary;
    ...
    for (int top_index : top_indices) {
        ...
        vocabulary.insert(partial_key_next(pair_mask, reference));
