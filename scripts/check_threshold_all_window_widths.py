...
# The score argument proves the criterion for every width. The finite loop below
# checks all 74,612 forced-alphabet multisets of widths one through sixteen.
...
for width in range(1, 17):
    for indices in combinations_with_replacement(range(len(SYMBOLS)), width):
        ...
        value = sum(score(SYMBOLS[index]) for index in indices)
        assert value == -2 * identity_count
        if identity_count == 0:
            assert all(legal(SYMBOLS[index]) for index in indices)
        else:
            assert value < 0
        forced_multisets_checked += 1
assert forced_multisets_checked == 74612
...
def gap_lengths(length, identity_positions):
    ...
    for index, position in enumerate(identities):
        ...
        gaps.append((next_position - position - 1) % length)
    assert sum(gaps) == length - len(identities)
    return tuple(gaps)


def all_window_counts(length, gaps):
    gap_histogram = [0] * (length + 1)
    for gap in gaps:
        gap_histogram[gap] += 1
    at_least = [0] * (length + 1)
    running = 0
    for value in range(length, 0, -1):
        running += gap_histogram[value]
        at_least[value] = running
    result = [0] * (length + 1)
    result[1] = sum(gaps)
    for width in range(1, length):
        result[width + 1] = result[width] - at_least[width]
    return result
...
for multiple in range(1, 4):
    ...
    for positions in combinations(range(length), identity_count):
        values = all_window_counts(length, gap_lengths(length, positions))
...
# One contiguous identity block leaves a single legal gap of length 5K and
# attains the formula simultaneously for all widths.
for multiple in range(1, 65):
    length = 8 * multiple
    values = all_window_counts(
        length,
        gap_lengths(length, range(3 * multiple)),
    )
    for width in range(1, length + 1):
        assert values[width] == max(0, 5 * multiple - width + 1)

print({
    "legal_permutation_layers": len(legal_layers),
    "forced_alphabet_size": len(SYMBOLS),
    "forced_multisets_checked_widths_1_through_16": forced_multisets_checked,
