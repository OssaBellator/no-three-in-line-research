#!/usr/bin/env python3
"""Audit BDA5ex--BDA5fb: canonical irreducible restoration cores."""
from collections import deque
import random
SEED, SYSTEMS = 18002, 4500
            ...
            def dfs(u,f):
                ...
                while it[u]<len(self.g[u]):
                    ...
                    if c and level[v]==level[u]+1:
                        z=dfs(v,min(f,c))
                        if z: self.g[u][i][1]-=z; self.g[v][r][1]+=z; return z
                    it[u]+=1
                return 0
            while True:
                z=dfs(s,10**9)
...
def sample(rng):
    ...
    return p,m,t,pc,pm,mt,[rng.randint(1,6) for _ in range(t)]

def capacity(a,mask):
    p,m,t,pc,pm,mt,d=a; s=0; ps=1; ms=ps+p; ts=ms+m; sink=ts+t; f=Flow(sink+1)
...
def main():
    rng=random.Random(SEED); z={k:0 for k in ("checks","deficient","addresses","deficit","marginal","single")}
    for _ in range(SYSTEMS):
        ...
        for mask in range(1,1<<t):
            ...
            if delta>0:
                addr=tuple(i for i in range(t) if mask>>i&1); bad.append((len(addr),addr,mask,delta))
        if not bad: continue
        z["deficient"]+=1; bad.sort(); _,addr,core,delta=bad[0]
        z["addresses"]+=len(addr); z["deficit"]+=delta; z["single"]+=len(addr)==1
        sub=(core-1)&core
        while sub: assert mass(a,sub)==cap[sub]; sub=(sub-1)&core
        for x in addr:
            marginal=cap[core]-cap[core&~(1<<x)]; assert a[6][x]-marginal==delta; z["marginal"]+=marginal
    print("BDA5ex--BDA5fb audit passed")
    for label,key in (("systems",None),("terminal-subset checks","checks"),("deficient systems","deficient"),("irreducible core addresses","addresses"),("core deficit units","deficit"),("marginal payable-capacity units","marginal"),("singleton cores","single")):
        print(f"{label}: {(SYSTEMS if key is None else z[key]):,}")
