#!/usr/bin/env python3
"""Verify the AC occurrence-slot assignment Hall-core interface."""
from random import Random

SEED, SYSTEMS, TYPED, P = 2501, 2600, False, 0.43
...
def main():
    ...
    st = {key: 0 for key in keys}
    st.update(max_deficit=0, max_core_size=0)
    for _ in range(SYSTEMS):
        ...
        nu = matching(adj, m)
        st["uses"] += n; st["slots"] += m; st["edges"] += sum(map(len, adj))
        st["subset_checks"] += 1 << n
        best, candidates = -1, []
        for mask in range(1 << n):
            deficit, _, _ = subset_data(mask, adj)
            if deficit > best:
                best, candidates = deficit, [mask]
            elif deficit == best:
        ...
        if best == 0:
            st["perfect"] += 1
            continue
        st["deficient"] += 1; st["deficit_units"] += best
        st["max_deficit"] = max(st["max_deficit"], best)
        core = min(candidates, key=lambda x: (x.bit_count(), tuple(i for i in range(n) if (x >> i) & 1)))
        deficit, core_uses, core_slots = subset_data(core, adj)
        assert deficit == best
        st["core_uses"] += core_uses; st["core_slots"] += core_slots
        st["max_core_size"] = max(st["max_core_size"], core_uses)
        st["singleton_cores"] += core_uses == 1
        if TYPED and len({types[i] for i in range(n) if (core >> i) & 1}) > 1:
            st["mixed_typed_cores"] += 1
        sub = core
        while True:
            st["proper_subset_checks"] += 1
            if sub != core:
                assert subset_data(sub, adj)[0] < best
            if sub == 0:
                break
            sub = (sub - 1) & core
        for use in range(n):
        ...
        core_adj = [adj[i] for i in range(n) if (core >> i) & 1]
        assert matching(core_adj, m) == core_slots
    print("AC occurrence-slot assignment Hall-core audit")
    print(f"systems={SYSTEMS}")
    for key in (*keys, "max_deficit", "max_core_size"):
        print(f"{key}={st[key]}")
