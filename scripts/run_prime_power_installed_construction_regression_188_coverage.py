#!/usr/bin/env python3
"""Run the 188-kind stack with the CMR691--747 inherited-coverage audit."""
...
class ConstructionRegression188CoverageError(RuntimeError):
    pass


def require(ok: bool, message: str) -> None:
    if not ok:
        raise ConstructionRegression188CoverageError(message)
...
BASE_CHAINED_MANIFEST_SHA256 = "7e93cf1e7379a12d4146f13a98d075225a9616321838f17002a9bc6ed120a6ca"
BASE_MANIFEST_COUNT = 38
MANIFEST_EXTENSION = [
    (
...
EXPECTED_CHAINED_MANIFEST_SHA256 = "7ac023aecd548ee7c966e8934b149d551a57a30e5569094abd6b26cf8bd29de1"
...
def repository_root(start: Path | None = None) -> Path:
    ...
    for candidate in (current, *current.parents):
        if (candidate / "STATUS.md").is_file() and (candidate / "scripts").is_dir():
            return candidate
    raise ConstructionRegression188CoverageError("unable to locate repository root")
...
def load_base_manifest(root: Path) -> list[tuple[str, str, str]]:
    runner188 = load_module(
        root / "scripts/run_prime_power_installed_construction_regression_188.py",
        "installed_regression_188",
    )
    require(
        runner188.EXPECTED_CHAINED_MANIFEST_SHA256 == BASE_CHAINED_MANIFEST_SHA256,
        "base 188-kind chained seal",
    )
    manifest = [*runner188.load_base_manifest(root), *runner188.MANIFEST_EXTENSION]
    runner188.validate_manifest(manifest)
    require(len(manifest) == BASE_MANIFEST_COUNT, "base thirty-eight-checker count")
    return manifest
...
def validate_manifest(manifest: list[tuple[str, str, str]]) -> None:
    ...
    require(len(manifest) == BASE_MANIFEST_COUNT + len(MANIFEST_EXTENSION),
            "thirty-nine-checker manifest required")
    paths: set[str] = set()
    flags: set[str] = set()
...
def audit(root: Path, item: tuple[str, str, str], execute: bool) -> dict[str, Any]:
    ...
    try:
        ...
        raise ConstructionRegression188CoverageError(f"{path}: invalid JSON report") from exc
...
def main() -> None:
    ...
    report = {
        "checker": "prime-power-installed-construction-regression-188-coverage",
        "base_chained_manifest_sha256": BASE_CHAINED_MANIFEST_SHA256,
