...
int main(int argc, char** argv) {
    if (argc != 2) {
        std::cerr << "usage: semantic-cover-core-index TOP_INDEX\n";
        return 64;
    }
    int top_index = std::stoi(argv[1]);
    const std::set<int> allowed = {0,1,6,7,9,50,51,56,57,60,61,63};
    if (!allowed.count(top_index)) return 64;
    ...
    assert(minimized[0].syntactic_mask == 6975);
    assert(minimized[1].syntactic_mask == 6975);
