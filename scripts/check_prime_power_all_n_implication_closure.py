...
The checker composes the independently derived global-family skeleton, spanning state-equivalence
evidence and global support-condensation certificate. It then validates a fixed proof-obligation DAG.
This is documentary closure infrastructure and permanently reports ``all_n_proved_by_checker = 0``.
"""
...
OBLIGATION_DEPENDENCIES: dict[str, tuple[str, ...]] = {
    ...
    "DESTROYED_RESOURCE_MODEL_EXHAUSTIVE": (
        "ACTIVE_ROW_FAMILY_EXHAUSTIVE", "GEOMETRY_SELECTOR_CORRECT"
    ),
    "CREDIT_ROUTING_SEMANTIC": (
        "DESTROYED_RESOURCE_MODEL_EXHAUSTIVE", "FATE_TRANSITION_STATE_SEMANTICS"
    ),
    "CLOSED_STRICT_RECURRENT_BLOCKS": (
        "CANDIDATE_POLICY_CORRECT", "CREDIT_ROUTING_SEMANTIC"
    ),
    ...
    "COMPONENT_SCALE_SEMANTIC": (
        "CROSS_BLOCK_STATE_IDENTITY_SEMANTIC", "CLOSED_STRICT_RECURRENT_BLOCKS"
    ),
...
def exact_obligation(record: dict[str, Any], obligation_id: str) -> dict[str, Any]:
    ...
    if status == "proved":
        ...
        require(artifact_locator is None,
                f"obligation {obligation_id}: open status requires null artifact locator")
        require(artifact_digest is None,
                f"obligation {obligation_id}: open status requires null artifact digest")
    output = {
...
def exact_certificate(certificate: dict[str, Any]) -> dict[str, Any]:
    ...
    require(isinstance(skeleton_certificate, dict),
            "global_family_skeleton_certificate: expected object")
    require(isinstance(equivalence_certificate, dict),
            "state_equivalence_evidence_certificate: expected object")
    ...
    id_sha = skeleton_certificate["global_integer_family_certificate"]["interface_row_certificate"][
        "weight_synchronization_certificate"]["state_identification_certificate"]["certificate_sha256"]
    ...
    require(len(raw_obligations) == len(expected_ids),
            "proof_obligations: exact fixed obligation count required")
    ...
    obligations = [
        exact_obligation(record, obligation_id)
        for record, obligation_id in zip(raw_obligations, expected_ids)
    ]
    require(raw_obligations == obligations, "proof_obligations: canonical records or digests required")

    closed_by_id: dict[str, int] = {}
    closure_records = []
    for record in obligations:
        obligation_id = record["obligation_id"]
        dependencies_closed = all(closed_by_id[dependency] for dependency in record["dependency_ids"])
        closed = int(record["status"] == "proved" and dependencies_closed)
        closed_by_id[obligation_id] = closed
        closure_record = {
            "obligation_id": obligation_id,
            ...
            "closed": closed,
            "open_dependency_ids": [
                dependency for dependency in record["dependency_ids"] if not closed_by_id[dependency]
            ],
    ...
    root_closed = closed_by_id["GLOBAL_QUOTIENT_IMPLIES_ALL_N"]
    all_n_readiness = int(interface_ready and root_closed)
    status_counts = Counter(record["status"] for record in obligations)
    open_ids = [record["obligation_id"] for record in obligations
                if not closed_by_id[record["obligation_id"]]]
    frontier_ids = [
        record["obligation_id"] for record in obligations
        if not closed_by_id[record["obligation_id"]]
        and all(closed_by_id[dependency] for dependency in record["dependency_ids"])
    ]
    claims = {
        ...
        "closed_obligations": sum(closed_by_id.values()),
        "unclosed_obligations": len(obligations) - sum(closed_by_id.values()),
    ...
    return {
        ...
        "obligation_closure_records": closure_records,
        "claims": claims,
    }
...
def validate_certificate(certificate: Any) -> dict[str, int]:
    ...
    claims = exact["claims"]
    return {
        "obligations": claims["proof_obligations"],
        "closed": claims["closed_obligations"],
        "frontier": claims["current_frontier_obligations"],
        "ready": claims["all_n_implication_dossier_ready"],
    }
