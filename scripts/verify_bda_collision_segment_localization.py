#!/usr/bin/env python3
"""Deterministic audit for BDA5fr--BDA5fv."""
from collections import defaultdict
from importlib import import_module
from random import Random
BASE = import_module("verify_bda_cut_arc_collision_pairs")
FLOW, UNIT_PATHS, ENABLED = BASE.FLOW, BASE.unit_paths, BASE.ENABLED
SEED, SYSTEMS = 2402, 2100

def make_system(rng):
    nt, np, nq, nr = rng.randint(2,4), rng.randint(2,3), rng.randint(2,3), rng.randint(2,3)
    s, p0, q0, r0 = 0, 1, 1+np, 1+np+nq
    x0, t, edges = r0+nr, r0+nr+nt, []
...
def segment(pa,pb,key):
    ...
    common=a[la:ra+1]; assert common==b[lb:rb+1]
...
def main():
    ...
    print("BDA collision-segment audit passed")
