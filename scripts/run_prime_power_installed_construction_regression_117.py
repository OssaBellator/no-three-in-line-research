#!/usr/bin/env python3
"""Run the installed one-hundred-seventeen-kind construction checker stack."""
...
class ConstructionRegression117Error(RuntimeError):
    pass


def require(ok: bool, message: str) -> None:
    if not ok:
        raise ConstructionRegression117Error(message)
...
BASE_CHAINED_MANIFEST_SHA256 = "bf0a209785183ec05e45b93bd9122159251571719bd63488240f34adacb23599"
BASE_MANIFEST_COUNT = 32
MANIFEST_EXTENSION = [
    (
        ...
        "selector_protected_certificate_ancestry_proved",
    ),
    (
        ...
        "installed_transition_kind_bank_117_exhaustive",
    ),
]
EXPECTED_CHAINED_MANIFEST_SHA256 = "d259fb78a7b8a2ccbf7387c3fdd7a780b1e21f0366179c08af5ebb068cda3507"
...
def repository_root(start: Path | None = None) -> Path:
    ...
    for candidate in (current, *current.parents):
        if (candidate / "STATUS.md").is_file() and (candidate / "scripts").is_dir():
            return candidate
    raise ConstructionRegression117Error("unable to locate repository root")
...
def load_base_manifest(root: Path) -> list[tuple[str, str, str]]:
    runner98 = load_module(
        root / "scripts/run_prime_power_installed_construction_regression_98.py",
        "installed_regression_98",
    )
    require(
        runner98.EXPECTED_CHAINED_MANIFEST_SHA256 == BASE_CHAINED_MANIFEST_SHA256,
        "base 98-kind chained seal",
    )
    manifest = [*runner98.load_base_manifest(root), *runner98.MANIFEST_EXTENSION]
    runner98.validate_manifest(manifest)
    require(len(manifest) == BASE_MANIFEST_COUNT, "base thirty-two-checker count")
    return manifest
...
def validate_manifest(manifest: list[tuple[str, str, str]]) -> None:
    ...
    require(len(manifest) == BASE_MANIFEST_COUNT + len(MANIFEST_EXTENSION),
            "thirty-four-checker manifest required")
    paths: set[str] = set()
    flags: set[str] = set()
...
def audit(root: Path, item: tuple[str, str, str], execute: bool) -> dict[str, Any]:
    ...
    try:
        ...
        raise ConstructionRegression117Error(f"{path}: invalid JSON report") from exc
...
def main() -> None:
    ...
    report = {
        "checker": "prime-power-installed-construction-regression-117",
        "base_chained_manifest_sha256": BASE_CHAINED_MANIFEST_SHA256,
