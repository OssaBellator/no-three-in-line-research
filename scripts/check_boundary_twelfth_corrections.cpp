...
int main(){
 ...
 int attempts=0, successes=0, cores_total=0;
 for(auto const&node:nodes)for(int off=-64;off<=64;off++){
   vector<Pt>block;for(auto p:node.pts)block.push_back({44+p.x,193+off+p.y});vector<Pt>all=T;all.insert(all.end(),block.begin(),block.end());sort(all.begin(),all.end());all.erase(unique(all.begin(),all.end()),all.end());auto locate=[&](Pt p){return int(lower_bound(all.begin(),all.end(),p)-all.begin());};set<array<int,3>>es;
   for(int i=0;i<(int)T.size();i++)for(int j=i+1;j<(int)T.size();j++)for(auto c:block)if(cross(T[i],T[j],c)==0){array<int,3>e={locate(T[i]),locate(T[j]),locate(c)};sort(e.begin(),e.end());es.insert(e);}for(int i=0;i<(int)block.size();i++)for(int j=i+1;j<(int)block.size();j++)for(auto c:T)if(cross(block[i],block[j],c)==0){array<int,3>e={locate(block[i]),locate(block[j]),locate(c)};sort(e.begin(),e.end());es.insert(e);}Solver s;s.n=all.size();s.edges.assign(es.begin(),es.end());int m=s.minimum(9);if(m>4)continue;attempts++;s.sel.assign(s.n,0);set<vector<int>>sets;s.enum_exact(m,sets);cores_total+=sets.size();cerr<<node.k<<node.v<<" "<<off<<" min="<<m<<" cores="<<sets.size()<<"\n";
   ...
   for(auto const&ixs:sets){vector<Pt>core;set<int>ci(ixs.begin(),ixs.end());for(int ix:ixs)core.push_back(all[ix]);vector<Pt>avail;for(int i=0;i<(int)all.size();i++)if(!ci.count(i))avail.push_back(all[i]);
     for(int budget=m;budget<=7;budget++){
       int e=budget-m;bool found=false;vector<Pt>del,add;
       if(e==0){del=core;found=refill_search(all,del,add);}else if(e==1){for(int a=0;a<(int)avail.size()&&!found;a++){del=core;del.push_back(avail[a]);found=refill_search(all,del,add);}}else if(e==2){for(int a=0;a<(int)avail.size()&&!found;a++)for(int b=a+1;b<(int)avail.size()&&!found;b++){del=core;del.push_back(avail[a]);del.push_back(avail[b]);found=refill_search(all,del,add);}}else if(e==3){for(int a=0;a<(int)avail.size()&&!found;a++)for(int b=a+1;b<(int)avail.size()&&!found;b++)for(int c=b+1;c<(int)avail.size()&&!found;c++){del=core;del.push_back(avail[a]);del.push_back(avail[b]);del.push_back(avail[c]);found=refill_search(all,del,add);}}else if(e==4){for(int a=0;a<(int)avail.size()&&!found;a++)for(int b=a+1;b<(int)avail.size()&&!found;b++)for(int c=b+1;c<(int)avail.size()&&!found;c++)for(int d=c+1;d<(int)avail.size()&&!found;d++){del=core;del.push_back(avail[a]);del.push_back(avail[b]);del.push_back(avail[c]);del.push_back(avail[d]);found=refill_search(all,del,add);}}
       if(found){successes++;attempt_success=true;sort(del.begin(),del.end());cout<<node.k<<node.v<<" "<<off<<" min "<<m<<" budget "<<budget<<"\nD";for(auto p:del)cout<<" ("<<p.x<<","<<p.y<<")";cout<<"\nA";for(auto p:add)cout<<" ("<<p.x<<","<<p.y<<")";cout<<"\n";goto next_attempt;}
     }
   }
   next_attempt: if(!attempt_success) cerr<<"NO_CORRECTION "<<node.k<<node.v<<" "<<off<<"\n";
 }
 cerr<<"attempts="<<attempts<<" cores="<<cores_total<<" successes="<<successes<<"\n";
}
