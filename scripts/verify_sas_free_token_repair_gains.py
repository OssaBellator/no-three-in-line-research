#!/usr/bin/env python3
"""Finite checks for SAS5ns--SAS5nv."""
from collections import Counter, defaultdict, deque
import random

def run(seed=109,cases=2500):
 ...
 for _ in range(cases):
  ...
  qn=r.randint(1,6);L=[(a,t,r.randrange(max(1,m*len(Z)//3)),r.randrange(qn)) for a in U for t in Z];C=Counter(p for a,t,w,p in L);p=max(C,key=C.get);F=[e for e in L if e[3]==p];assert len(F)*qn>=m*len(Z);a,t,w,p=F[0];assert len(M|{(a,t)})==len(M)+1
  r.shuffle(F);ur,ut,N=set(),set(),[]
  for e in F:
   a,t,w,p=e
