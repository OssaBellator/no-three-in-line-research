...
int verify_tier(int argc, char** argv, const std::array<CaseData,CaseCount>& cases, int multiplicity, int expected_signature_count, uint64_t expected_aggregate_bottom) {
    int requested_case = argc>1 ? std::stoi(argv[1]) : -1;
    int requested_orientation = argc>2 ? std::stoi(argv[2]) : -1;
    assert(requested_case < static_cast<int>(cases.size()));
    ...
    for (auto const& [sig,group]:groups) ++histogram[static_cast<int>(group.size())];
    assert(histogram[multiplicity]==expected_signature_count);
    ...
    for (int case_index=0;case_index<static_cast<int>(cases.size());++case_index) {
        ...
        auto group=it->second; std::sort(group.begin(),group.end()); assert(static_cast<int>(group.size())==multiplicity);
        TopEnumerator top[2];
        for(int mode=0;mode<2;++mode) { top[mode].sig=data.signature; top[mode].run(mode); assert(static_cast<int>(top[mode].solutions.size())==data.top_orders[mode]); assert(top[mode].nodes==data.top_nodes[mode]); }
    ...
    if(requested_case<0 && requested_orientation<0) { assert(checked==static_cast<int>(cases.size())); assert(aggregate_bottom==expected_aggregate_bottom); std::cout<<"aggregate_bottom_nodes="<<aggregate_bottom<<" PASS\n"; }
    return 0;
}
