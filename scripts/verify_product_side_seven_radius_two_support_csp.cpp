...
struct CSP{
 ...
 static std::array<std::array<Mask,196>,196> lines;
 static bool lines_ready;
 ...
 bool candidate_ok(const Mask& danger,const std::array<uint8_t,2>&nw,int nc,std::array<uint8_t,2>&ids) const{
   if(nc==0)return true;
   ids[0]=calc_id(nw[0]); if(danger.has(ids[0]))return false;
   if(nc==2){ids[1]=calc_id(nw[1]);if(danger.has(ids[1]))return false;for(int i=0;i<compCount;i++)if(lines[ids[0]][pointId[compList[i]]].has(ids[1]))return false;}
   return true;
 }
 ...
 bool search(int depth,const Mask&danger){++nodes;if(depth==21)return true;int bv=-1,bc=8;std::array<int8_t,7>dom{};int dn=0;
  for(int v=0;v<21;v++)if(asg[v]<0){std::array<int8_t,7>d{};int c=0;for(int val=0;val<7;val++)if(allowed(v,val,danger))d[c++]=val;if(c==0)return false;if(c<bc){bc=c;bv=v;dn=c;dom=d;if(c==1)break;}}
  int g=group(bv);for(int ii=0;ii<dn;ii++){int val=dom[ii];asg[bv]=val;used[g]|=1u<<val;std::array<uint8_t,2>nw{},ids{};int nc=newly_for(bv,nw);bool ok=candidate_ok(danger,nw,nc,ids);assert(ok);Mask child=danger;
   for(int k=0;k<nc;k++){int pi=nw[k],id=ids[k];for(int i=0;i<compCount;i++)child.add(lines[id][pointId[compList[i]]]);for(int q=0;q<k;q++)child.add(lines[id][ids[q]]);complete[pi]=1;pointId[pi]=id;compList[compCount++]=pi;}
   if(search(depth+1,child))return true;
   for(int k=nc-1;k>=0;k--){--compCount;complete[nw[k]]=0;}used[g]&=~(1u<<val);asg[bv]=-1;
  }return false;
 }
