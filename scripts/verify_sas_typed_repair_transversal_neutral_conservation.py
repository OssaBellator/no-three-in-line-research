#!/usr/bin/env python3
"""Deterministic replay audit for SAS5mi--SAS5mm."""
from collections import Counter
from random import Random

FAULTS=("source_less","split","reuse","hidden_deposit","class_mismatch")
SEED,SYSTEMS=2706,2700
EXPECTED={"systems":2700,"initial":20400,"events":53583,"deposits":18590,"transitions":8257,"selected":26437,"class_checks":271875,"invalid":1506,"valid":1194}
EXPECTED_FAULTS={"reuse":303,"source_less":295,"hidden_deposit":299,"split":319,"class_mismatch":290}
EXPECTED_TYPES={1:13261,0:13176}
...
def system(rng,sid):
    ...
    for _ in range(rng.randint(4,9)):
        for _ in range(rng.randint(0,2)):
            t=f"s{sid}_{nxt}"; nxt+=1; c=rng.randrange(k); events.append(("deposit",t,c,None)); live[t]=c
        ...
        for t in rng.sample(sorted(live),min(len(live),rng.randint(0,3))):
            c=live.pop(t); events.append(("debit",t,c,rng.randrange(2)))
    fault=None
    if rng.random()<.55:
        fault=rng.choice(FAULTS)
        if fault=="source_less": events.append(("debit",f"ghost{sid}",rng.randrange(k),rng.randrange(2)))
        elif fault=="split":
            t=f"s{sid}_{nxt}"; nxt+=1; c=rng.randrange(k); events.append(("deposit",t,c,None)); a=f"s{sid}_{nxt}"; nxt+=1; b=f"s{sid}_{nxt}"; nxt+=1; events += [("transition",t,a,c),("transition",t,b,c)]
        elif fault=="reuse":
            t=f"s{sid}_{nxt}"; nxt+=1; c=rng.randrange(k); typ=rng.randrange(2); events += [("deposit",t,c,None),("debit",t,c,typ),("debit",t,c,typ)]
        elif fault=="hidden_deposit":
            t=f"s{sid}_{nxt}"; c=rng.randrange(k); events += [("hidden",t,c,None),("debit",t,c,rng.randrange(2))]
        else:
            t=f"s{sid}_{nxt}"; c=rng.randrange(k); events += [("deposit",t,c,None),("debit",t,(c+1)%k,rng.randrange(2))]
...
def main():
    rng=Random(SEED); totals=Counter(); faults=Counter(); types=Counter()
    for sid in range(SYSTEMS):
        ...
        for e in events:
            if e[0]=="debit":types[e[3]]+=1
        if observed:faults[observed]+=1
    assert dict(totals)==EXPECTED and dict(faults)==EXPECTED_FAULTS and dict(types)==EXPECTED_TYPES
    print("SAS typed repair-neutral audit:",dict(totals)); print("typed debits:",dict(types)); print("first failures:",dict(faults))
