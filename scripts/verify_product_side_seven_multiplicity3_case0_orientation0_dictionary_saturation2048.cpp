...
std::vector<int> m3_certificate2048_edges(State const& state) {
...
Point m3_certificate2048_point(int edge,Assignment const& top,std::array<int8_t,N> const& bottom) {
    int row=edge/SIDE;
    int column=edge%SIDE;
    return {row<N ? row : N+bottom[row%N],N*(column/N)+top[column]};
}

std::array<uint8_t,3> m3_certificate2048_first_bad(State const& state,Assignment const& top,std::array<int8_t,N> const& bottom) {
    auto edges=m3_certificate2048_edges(state);
    for(size_t i=0;i<edges.size();++i)
        for(size_t j=i+1;j<edges.size();++j)
            for(size_t k=j+1;k<edges.size();++k)
                if(det(m3_certificate2048_point(edges[i],top,bottom),m3_certificate2048_point(edges[j],top,bottom),m3_certificate2048_point(edges[k],top,bottom))==0)
                    return {static_cast<uint8_t>(edges[i]),static_cast<uint8_t>(edges[j]),static_cast<uint8_t>(edges[k])};
    assert(false);
    return {};
}

uint64_t m3_saturation2048_mix(uint64_t hash,uint64_t value) {return (hash^value)*1099511628211ULL;}
...
int main() {
    ...
    TopEnumerator top;top.sig=signature;top.run(0);assert(top.solutions.size()>=2048);
    std::set<std::array<uint8_t,3>> dictionary;
    uint64_t digest=1469598103934665603ULL;
    size_t obligations=0;
    for(int top_index=0;top_index<2048;++top_index) {
        auto const& top_assignment=top.solutions[top_index];
        digest=m3_saturation2048_mix(digest,top_index);
        for(auto value:top_assignment) digest=m3_saturation2048_mix(digest,static_cast<uint8_t>(value));
        ...
        do {
            for(int selector=0;selector<3;++selector) {
                auto triple=m3_certificate2048_first_bad(group[selector],top_assignment,bottom);
                dictionary.insert(triple);
                for(auto edge:triple) digest=m3_saturation2048_mix(digest,edge);
                ++obligations;
            }
    ...
    size_t projected_bytes=48+2048*14+dictionary.size()*3+obligations;
    size_t raw_bytes=48+2048*14+3*obligations;
    assert(dictionary.size()==1202);
    assert(obligations==30965760);
    assert(projected_bytes==30998086);
    assert(raw_bytes==92926000);
    assert(digest==8200777914864976615ULL);
