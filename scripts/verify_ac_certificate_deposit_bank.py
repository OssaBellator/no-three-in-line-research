...
def assignment(dem,cap,edges):
    nl,nr=len(dem),len(cap); s=nl+nr; t=s+1; D=Dinic(t+1); em={}
    for i,x in enumerate(dem): D.add(s,i,x)
    for i,j in sorted(edges): D.add(i,nl+j,sum(dem)+1); em[(i,j)]=D.g[i][-1]
    for j,x in enumerate(cap): D.add(nl+j,t,x)
    f=D.flow(s,t); used=[0]*nr
    for (i,j),e in em.items(): used[j]+=e[2][1]
    return f,used

def hall(dem,cap,edges):
    N=[set() for _ in dem]
    for i,j in edges: N[i].add(j)
    best=0
...
rng=random.Random(20260728); S=defaultdict(int)
for _ in range(6000):
    nl,nr=rng.randint(1,6),rng.randint(1,6)
    cap=[rng.randint(0,5) for _ in range(nr)]; initial=cap[:]; depcum=[0]*nr; usedcum=[0]*nr
    E={(i,j) for i in range(nl) for j in range(nr) if rng.random()<.55}
    S['systems']+=1; S['layers']+=nl; S['certificates']+=nr; S['arcs']+=len(E)
    for _ in range(rng.randint(1,4)):
        dep=[rng.randint(0,2) if rng.random()<.45 else 0 for _ in range(nr)]
        for j,x in enumerate(dep): cap[j]+=x; depcum[j]+=x
        dem=[rng.randint(0,4) for _ in range(nl)]
        f,used=assignment(dem,cap,E); deficit=hall(dem,cap,E)
