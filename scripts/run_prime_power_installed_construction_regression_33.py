#!/usr/bin/env python3
"""Run the installed thirty-three-kind construction checker stack.

This superseding manifest contains nineteen checkers, including the Hall-wall,
rollback restoration, rollback optimal-face/SCC and both registry-extension
layers.  It validates installed finite claims only.
"""
...
class ConstructionRegression33Error(RuntimeError):
    pass


def require(ok: bool, message: str) -> None:
    if not ok:
        raise ConstructionRegression33Error(message)
...
EXPECTED_MANIFEST_SHA256 = "50b66679af79082ff57ec0926508482f1efb72eefd6fe0e620d41eb949890fca"
...
def validate_manifest() -> None:
    ...
    for i, item in enumerate(CHECKER_MANIFEST):
        ...
        require(len(contract) == 64 and all(c in "0123456789abcdef" for c in contract),
                f"manifest[{i}]: invalid contract")
        require(flag and flag not in flags, f"manifest[{i}]: duplicate flag")
        flags.add(flag)
...
def parse_report(stdout: str, path: str) -> dict[str, Any]:
    text = stdout.strip()
    require(bool(text), f"{path}: empty stdout")
    try:
        report = json.loads(text)
    except json.JSONDecodeError as exc:
        raise ConstructionRegression33Error(
...
def main() -> None:
    ...
    report = {
        "checker": "prime-power-installed-construction-regression-33",
