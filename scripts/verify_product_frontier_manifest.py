...
def main() -> None:
    ...
    require(manifest["certified_through"] == "PX1210", "unexpected certified theorem boundary")
    ...
    require((infeasible, constructive, unclassified) == (40_439, 2, 31_419), "side-seven totals mismatch")
    require(infeasible + constructive + unclassified == total, "side-seven selector partition mismatch")
    require(as_int(side7, "certified_rejection_nodes") == 3_293_788_263, "rejection-node boundary mismatch")

    classified_cases = side7["multiplicity_two_classified_cases"]
    require(classified_cases == [0, 1419], "unexpected multiplicity-two classified interval")
    ...
    require(classified_selectors == 2 * classified_case_count == 2_840, "multiplicity-two selector count mismatch")
    require(
    ...
    require(as_int(side7, "multiplicity_two_infeasible_selectors") == 2_839, "m2 infeasible count mismatch")
    ...
    require(side7["multiplicity_two_next_case"] == 1420, "unexpected next multiplicity-two case")
    require(side7["registered_uncounted_cases"] == [1420, 1429], "unexpected registered side-seven range")
    ...
    require(classified_indices == [0, 5999], "unexpected side-ten classified interval")
    geometry_count = classified_indices[1] - classified_indices[0] + 1
    require(as_int(side10, "fc_geometries") == geometry_count == 6_000, "fc geometry count mismatch")
    require(as_int(side10, "ff_geometries") == geometry_count == 6_000, "ff geometry count mismatch")
    require(as_int(side10, "fc_nodes") == 222_250_906, "fc node count mismatch")
    require(as_int(side10, "ff_nodes") == 131_624_531, "ff node count mismatch")
    ...
    require(side10["registered_uncounted_indices"] == [6000, 6399], "unexpected registered side-ten range")
    ...
    require_markers(
        "docs/370-side-seven-multiplicity-two-cases-1410-through-1419.md",
        [
            "`40,439` certified-infeasible selectors",
            "`31,419` unclassified selectors",
            "`3,293,788,263` certified rejection-CSP nodes",
            "next canonical multiplicity-two case is `1420`",
            "`527,333,019` certified rejection-CSP nodes",
        ],
    )
    require_markers(
        "docs/371-side-ten-opposite-pair-fine-row-fifteenth-prefix.md",
        [
            "pair indices `0` through `5999`",
            "`222,250,906` nodes",
            "`131,624,531` nodes",
            "next bounded prefix begins at pair index `6000`",
        ],
    )
    require_markers(
    ...
    require_markers(
        "STATUS.md",
        [
            "Cases `1420--1429` are registered",
    ...
    require_markers(
        ...
        [
            "through PX1210",
            "Cases `1420--1429` are registered",
            "Pair indices `6000--6399` are registered but uncounted",
            "`16150749401146711547`",
        ],
    )
    require_markers(
        ...
        [
            "Certified theorem boundary: `PX1210`",
            "The next canonical multiplicity-two case is `1420`",
            "next bounded prefix begins at pair index `6000`",
    ...
    require_markers(
        ...
        [
            "case: [1420, 1421, 1422, 1423, 1424, 1425, 1426, 1427, 1428, 1429]",
            "run-side-seven-multiplicity-two-1420-1429",
            "cancel-in-progress: true",
            "Registered but uncounted",
        ],
    )
    require_markers(
        ".github/workflows/product-side-ten-opposite-fine-6000-6399.yml",
        [
            "orientation: [fc, ff]",
            "first_pair: [6000, 6100, 6200, 6300]",
            "run-opposite-fine-6000-6399",
            "cancel-in-progress: true",
            "Registered but uncounted",
        ],
    )
    require_markers(
        ...
        [
            ...
            "verify_product_side_seven_multiplicity2_cases1410_1419.py",
            ...
            "verify_product_transposition_double_coset_opposite_fine_ten_5600_5999.py",
            "verify_product_side_seven_multiplicity2_case0_orientation3_semantic_uncovered16.py",
            "cancel-in-progress: true",
        ],
    )
    require_markers(
        ".github/product-side-seven-multiplicity-two-1420-1429-trigger.txt",
        ["run-side-seven-multiplicity-two-1420-1429"],
    )
    require_markers(
        ".github/product-side-ten-opposite-fine-6000-6399-trigger.txt",
        ["run-opposite-fine-6000-6399"],
    )
    require_missing(".github/product-side-seven-multiplicity-two-1410-1419-trigger.txt")
    require_missing(".github/product-side-ten-opposite-fine-5600-5999-trigger.txt")
    require_missing(".github/workflows/product-side-ten-opposite-fine-5600-5999.yml")
    require_missing(".github/workflows/product-side-seven-semantic-uncovered16.yml")

    print(
        "PX1210 frontier manifest: "
        "side7=40439+2+31419 "
        "side7_registered=1420--1429 "
        "semantic=208refs,165keys,221covered,34891uncovered "
        "side10_certified=0--5999 "
        "side10_registered=6000--6399 "
        "frontiers=8 PASS"
    )
