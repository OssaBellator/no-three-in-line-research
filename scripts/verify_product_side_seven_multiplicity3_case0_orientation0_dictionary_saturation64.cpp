...
std::vector<int> m3_certificate64_edges(State const& state) {
...
Point m3_certificate64_point(int edge,Assignment const& top,std::array<int8_t,N> const& bottom) {
    int row=edge/SIDE;
    int column=edge%SIDE;
    return {row<N ? row : N+bottom[row%N],N*(column/N)+top[column]};
}

std::array<uint8_t,3> m3_certificate64_first_bad_triple(State const& state,Assignment const& top,std::array<int8_t,N> const& bottom) {
    auto edges=m3_certificate64_edges(state);
    for(size_t i=0;i<edges.size();++i)
        for(size_t j=i+1;j<edges.size();++j)
            for(size_t k=j+1;k<edges.size();++k)
                if(det(m3_certificate64_point(edges[i],top,bottom),m3_certificate64_point(edges[j],top,bottom),m3_certificate64_point(edges[k],top,bottom))==0)
                    return {static_cast<uint8_t>(edges[i]),static_cast<uint8_t>(edges[j]),static_cast<uint8_t>(edges[k])};
    assert(false);
    return {};
}

uint64_t m3_saturation64_mix(uint64_t hash,uint64_t value) {return (hash^value)*1099511628211ULL;}
...
int main() {
    ...
    for(int top_index=0;top_index<64;++top_index) {
        ...
        digest=m3_saturation64_mix(digest,top_index);
        for(auto value:top_assignment) digest=m3_saturation64_mix(digest,static_cast<uint8_t>(value));
        ...
        do {
            for(int selector=0;selector<3;++selector) {
                auto triple=m3_certificate64_first_bad_triple(group[selector],top_assignment,bottom);
                dictionary.insert(triple);
                for(auto edge:triple) digest=m3_saturation64_mix(digest,edge);
                ++obligations;
            }
    ...
    assert(dictionary.size()==283);
    assert(obligations==967680);
    assert(projected_bytes==969473);
    assert(digest==1254626597863958761ULL);
