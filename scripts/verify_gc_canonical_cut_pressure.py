#!/usr/bin/env python3
"""Finite audit for GC2kf--GC2kj."""
from collections import deque
from random import Random

SEED, SYSTEMS, TYPED = 2104, 3000, False
...
def system(rng):
    ...
    types = [rng.randrange(2) for _ in range(nt)] if TYPED else [0] * nt
    return t + 1, edges, s, t, demand, types
...
def main():
    rng = Random(SEED)
    out = dict(systems=SYSTEMS, subset_checks=0, deficient=0, singletons=0, nontrivial=0,
               cut_arcs=0, positive_arcs=0, deficit_units=0, nontrivial_deficit=0,
               positive_pressure=0, backward_crossings=0, typed_mixed=0)
    for _ in range(SYSTEMS):
        ...
        a = sum(1 << k for k in members[:len(members) // 2]); b = core ^ a
        assert vals[a][1] == vals[b][1] == 0
    ...
    print("GC canonical cut-pressure audit passed")
