#!/usr/bin/env python3
"""Finite checks for BDA5hz--BDA5ic."""
from collections import Counter
from math import ceil
from random import Random

SEED, TRIALS = 733, 2500


def independent(fps):
    left=set(range(len(fps))); out=[]
    while left:
        v=min(left); out.append(v)
        left-={u for u in left if fps[u]&fps[v]}
    return out
...
def verify():
    ...
    for _ in range(TRIALS):
        ...
        for _ in range(r.randint(1,35)):
            avail=[x for x in range(p) if load[x]<beta]
            if not avail: break
            fp=set(r.sample(avail,r.randint(1,min(h,len(avail)))))
            for x in fp: load[x]+=1
            fps.append(fp)
        if not fps: continue
        assert max(Counter(x for f in fps for x in f).values())<=beta
        bound=h*(beta-1)
        for i,f in enumerate(fps):
            assert sum(i!=j and bool(f&g) for j,g in enumerate(fps))<=bound
        I=independent(fps)
        ...
        C=[sum(costs[i][a] for i in I) for a in range(d)]
        B=[r.randint(0,c+3) for c in C]
...
if __name__=='__main__':
    print('BDA footprint-cost checks passed:', *verify())
