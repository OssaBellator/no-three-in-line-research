...
int main(){
 ...
 for(auto const&node:nodes){
  for(int off=-64;off<=64;off++){
   vector<Pt> block;for(auto p:node.pts)block.push_back({44+p.x,193+off+p.y});
   ...
   if(m<=8){s.sel.assign(s.n,0);set<vector<int>>sets;s.enum_exact(m,sets);small++;totalsets+=sets.size();cout<<node.k<<node.v<<" "<<off<<" min "<<m<<" triples "<<eset.size()<<" sets "<<sets.size()<<"\n";}
  }
 }
