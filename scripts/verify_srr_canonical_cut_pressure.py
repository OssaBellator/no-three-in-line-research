#!/usr/bin/env python3
"""Finite audit for SRR2do--SRR2ds."""
from collections import deque
from random import Random
SEED, SYSTEMS = 2106, 3200
...
def system(rng):
    ...
    return t+1,edges,s,t,demand

def enabled(demand,mask): return {("term",k):d if mask>>k&1 else 0 for k,d in enumerate(demand)}

def main():
    rng=Random(SEED)
    out=dict(systems=SYSTEMS,subset_checks=0,deficient=0,singletons=0,nontrivial=0,cut_arcs=0,positive_arcs=0,deficit_units=0,nontrivial_deficit=0,positive_pressure=0,backward_crossings=0)
    for _ in range(SYSTEMS):
        n,edges,s,t,demand=system(rng); nt=len(demand); vals={}
        ...
        a=sum(1<<k for k in members[:len(members)//2]); b=core^a; assert vals[a][1]==vals[b][1]==0
    ...
    print("SRR canonical cut-pressure audit passed")
