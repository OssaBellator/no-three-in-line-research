#!/usr/bin/env python3
"""Verify the SAS typed occurrence-slot assignment Hall-core interface."""
from random import Random

SEED, SYSTEMS, TYPED, P = 2507, 2700, True, 0.42
...
def subset_data(mask, adj):
    ...
    for use, row in enumerate(adj):
        if (mask >> use) & 1:
            uses += 1; neighbours.update(row)
    return uses - len(neighbours), uses, len(neighbours)


def main():
    rng = Random(SEED)
    keys = ("uses", "slots", "edges", "perfect", "deficient", "deficit_units", "core_uses", "core_slots",
            "subset_checks", "proper_subset_checks", "essential_removals", "mixed_typed_cores", "singleton_cores")
    st = {key: 0 for key in keys}; st.update(max_deficit=0, max_core_size=0)
    for _ in range(SYSTEMS):
        n, m = rng.randint(1, 8), rng.randint(0, 9)
        types = [rng.randrange(2) for _ in range(n)]
        adj = []
        for _use in range(n):
            row = [slot for slot in range(m) if rng.random() < P]
            if m and not row and rng.random() < 0.55: row = [rng.randrange(m)]
        ...
        assert best == n - nu
        if best == 0: st["perfect"] += 1; continue
        ...
        if len({types[i] for i in range(n) if (core >> i) & 1}) > 1: st["mixed_typed_cores"] += 1
        sub = core
        while True:
    ...
    print("SAS typed occurrence-slot assignment Hall-core audit"); print(f"systems={SYSTEMS}")
