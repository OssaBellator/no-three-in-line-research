...
| SAS5ao–SAS5as | Distinct donor defects give distinct designated mirror records; global compatibility preserves a quantitatively large simultaneous designated-repair bank | PROVED UNDER THE GLOBAL COLUMN-INCIDENCE CAP `Lambda` | `docs/sparse-designated-mirror-repair-bank.md` |
| SAS5at–SAS5ax | In a double-scope defect role one donor repairs the whole weighted defect-column fibre; top fibres admit `d-2` donor matching and retain explicit weighted mass through global compatibility | PROVED UNDER THE GLOBAL COLUMN-INCIDENCE CAP `Lambda` FOR THE ADDITIVE SUBBATCH | `docs/sparse-double-scope-weighted-defect-fibres.md` |
| SAS6 | Arithmetic batching or classification of concentrated swap certificates | OPEN; DOUBLE-SCOPE REFLECTED DEFECTS NOW YIELD A WEIGHTED WHOLE-FIBRE REPAIR BATCH WITH EXPLICIT `d-2` AND `4Lambda+1` LOSSES, WITH SINGLETON FIBRE WEIGHT, ARITHMETIC CLASSIFICATION AND HIGH-INCIDENCE/BOUNDARY PROFILES REMAINING | `docs/sparse-algebraic-spread.md` |
