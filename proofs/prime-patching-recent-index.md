...
This focused index records the active controller-aware allocation and endpoint
trade frontier on `research/all-n-prime-patching`.
...
| PP3xp--PP3xv | Full-pool fillers dilute marked high-support source mass | PROVED | `docs/160-pool-filler-dilution-source-validity.md` |
...
Matching supply, degree restoration, internal macro geometry, patch-only external
energy, ordinary same-slot anchor energy, four allocation interfaces, rectangle
extraction, diffuse paid rectangle selection, and marked source dilution are
closed as implications. A saturation-compatible controller-aware allocation still
gives the full patch by PP3hq.
...
1. a genuinely nonvanishing insertion-load certificate;
...
Deterministic local-credit failure splits into seven existing roles. Fixed-centre
unique completion improves the marked anchored-pair and inserted-triple degrees
by one full power of `N`; after clean conditioning all high-support and transition
source terms vanish. Any nonvanishing residual source objective therefore yields
a marked or global target-size unary forbidden-cell star.
...
`O(sqrt(epsilon_c))`. Therefore every merely vanishing source/off-centre paid
...
unstructured rank-two unary `Xi` fibres, fixed-axis unary `Xi` stars, unstructured
