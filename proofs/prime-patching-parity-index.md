...
The strict-to-clean switch is exact for reachability and charge. A fixed repair
word has at most one predecessor, and all remaining merging is generated by the
weighted reverse indegree `kappa`. Over several strict steps the column bound is
the product of the stepwise `kappa` values; for a uniform policy with at least
`B` forward labels and at most `D` active inverse labels per target,
`kappa<=D/B`. Fibre regeneration and clean heat evolution can only contract
this inherited density further.
...
The `m=10` distance-five layer is now completely classified. It is the union of
the whole clean fibres over exactly 1,260 Hamilton cycles, and every supporting
parity graph is a forest. The joint minimum consists of sixteen states in eight
global-sign-complement pairs, each with two flaw supports and eight atomic
triples. Four states have shortest-path peak `(2,8)`; the other twelve have the
sharp joint peak `(3,12)`.

Exact weighted-Hall transport reaches `m=9` without enumerating clean signed
states. There are 25,540 atomic signed-assignment flaws, 25,520 with a proper
bottleneck. The worst charge is `223/29271`, or `5.55386/m^3` after scaling,
and its maximizing cut uses 77 of 96 source cycles. Thus neither global mass
nor constant-size expansion alone controls the finite obstruction.

## Remaining tasks

1. Prove asymptotic existence of pair-safe covering rotations with low-cost
   reduced boundary solutions.
2. Prove a geometric bound `D/B<1`, or a weighted analogue, for an admissible
   strict-repair policy throughout the logarithmic frustration window.
3. Generalize the explicit `m=9` and `m=10` sparse terminal banks to a structural
   bounded-collateral word with controlled predecessor charge.
4. Characterize the 1,260 extremal `m=10` Hamilton cycles by a cycle-coordinate
   invariant and prove a structural macro-horizon bound explaining `3,4,5`.
5. Prove uniform weighted expansion or heat-kernel control for local,
   intermediate, and global Hall cuts.
6. Extend compressed exact Hall transport to `m=10`, where one flaw has at most
   `6!=720` source cycles.
7. Combine covering local repair, reverse-collision contraction, inherited-load
   heat evolution, the owner light cone, and compensated three-owner drift.

The next available theorem identifier is `PP3bqt`.
