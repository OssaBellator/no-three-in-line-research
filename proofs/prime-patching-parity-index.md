...
The strict-to-clean charge interface is now exact at two levels. A fixed repair
word has at most one predecessor, and all word merging is generated by weighted
reverse indegrees. On the complete target-optimal fixed-sign strict graph,
uniform weighting develops terminal overloads at `m=8,9`, but exact Hall-balanced
weights give one-step charge at most `489/1726` through `m=9`. Since strict
frustration depth is at most three, the complete absorbing signed-column bound
is below `0.387`; fibre regeneration and clean heat evolution contract it
further. The open step is a geometric or asymptotic construction of these
balanced weights, and its locally coupled extension at `m=10`.
...
Clean-macro distance now has an exact cycle-coordinate shell recursion. A whole
fibre lies at distance `h` exactly when no eligibility mask reaches a cycle
layer below `h-1`, while the layer `h-1` neighbours cover the entire root cube.
At `m=10`, exactly 1,260 cycles satisfy the fifth-shell gate: they have no
active route into cycle minimum distance at most three, and their distance-four
neighbours cover every clean orientation. These fibres contain all `535,072`
distance-five states.
...
3. Extend reverse-collision optimisation to the locally coupled covering-repair
   graph at `m=10` and identify its sharp weighted reverse indegree.
...
5. Bound the clean-macro fibre-shell depth by a decreasing cycle invariant and
   explain the finite `3,4,5` progression asymptotically.
...
The next available theorem identifier is `PP3bra`.
