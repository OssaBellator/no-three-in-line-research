...
6. All side-seven support-twenty selectors of multiplicity at least three are classified. Multiplicity-two cases `0--1409` are also classified with witness-aware exact replay.
7. The classified multiplicity-two prefix contains `2,820` selectors: `2,819` infeasible and one constructive selector at case `1287`, selector zero, orientation zero.
8. Across support twenty, the exact cache contains `40,419` certified-infeasible selectors, two constructive selectors, `31,439` unclassified selectors, and `3,291,140,089` certified rejection-CSP nodes.
...
10. At side ten, the opposite-pair double coset is completely obstructed in `cc` and `cf`. Fine-row pair indices `0--5599` are obstructed in both `fc` and `ff`, with no constructive witness in those 11,200 geometries.
...
The next canonical multiplicity-two case is `1410`. Cases `1410--1419` are registered for exact computation but are not counted until every transcript is promoted and replayed.

For the side-ten opposite-pair fine-row search:

- `fc`: indices `0--5599`, `202,321,011` nodes, maximum `1,877,339`;
- `ff`: indices `0--5599`, `121,526,788` nodes, maximum `909,040`.

Indices `5600--5999` are registered but uncounted.

## Active frontiers

- Complete multiplicity two from case `1410`, then classify multiplicity one.
- Continue side-ten fine-row obstruction from pair index `5600`, then address the larger double cosets.
