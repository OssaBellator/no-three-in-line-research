...
conditional graph interfaces, and finite reservoirs are not all-length coordinate
constructions.
...
- Periodic schedules are classified by full-cycle gain and within-cycle prefix
  maxima.
...
- A deterministic low-defect search reaches it at depth fourteen after scoring
  884,907 valid states.
- Fourteen anchors pass all 8,192 compositions with maximum coordinate 80.
- Incidence component sizes are `2,2,2,2,3,3`, giving four induced twelve-pair and
  two induced eleven-pair subsources.
- For the older thirteen-pair source, all 208 two-component deletion cases have
  boundary flow `(2,2)`, rerouting radius four, and 144 optimal repairs each.

### Shell — `docs/661-shell-repertoire-envelope.md`

Theorems `PP3dat--PP3dav`.

- Static repertoire and lower-quota burdens have exact sharp minima.
- A finite compatibility graph amortizes every setup exactly when a reachable
  cycle has positive saving, equivalently mean burden below three.
- The best asymptotic rate is the maximum reachable cycle mean.
- Uniform improvement over every schedule occurs exactly when maximum repertoire
...
- Fourteenth, fifteenth, and sixteenth boundary spectra, correction kernels, and
...
- Unsuccessful exploratory searches were not committed as theorem evidence.
